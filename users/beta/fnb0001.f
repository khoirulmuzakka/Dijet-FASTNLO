...
*     FB0001CC        computes cross sections / fills arrays
*     FB0001RD        reads coefficient tables
*     FB0001PR        prints x-section results 
*     FB0001GP        gets the PDF values for all x-bins
*     FB0001MT        multiply PDF array and coefficient array
*
* uses commonblock definitions in: fnb0001.inc
*
...
*******************************************************************
      SUBROUTINE FB0001CC(FILENAME, IMUFFLAG, IPRINTFLAG, XSECT)
*-----------------------------------------------------------------
...
c         write (*,*) ' old/new table ',OLDFILENAME,'   ',FILENAME
         call FB0001RD(FILENAME)  ! read combined LO & NLO table
...
c - compute xsect for different scales
      if (IPRINTFLAG.eq.1) call FB0001PR(IMUFFLAG)
...
*-----------------------------------------------------------------
      ...
      DOUBLE PRECISION x1, x2, xlim, hx, hxlim, muf, H(7)
...
c           write(*,*) '>>> ',i,j, muffactor,mufval(i,j)
            ...
            do k=1,NTOT         ! Nxmax
               ...
               do l=1,k         ! Nxmin 
                  ...
                  nx=nx+1
                  call FNPAPDF(x1,x2,muf,H)  ! get PDFs at x1,x2 at scale muf
c                  call FNPPPDF(x1,x2,muf,H)  ! get PDFs at x1,x2 at scale muf
                  do m=1,7      ! Subprocess
                     pdf(nbin,nx,m) = H(m)
      ...
      do iscale=1,maxscale
         ...
         do i=1,nrapidity       ! Rapidity Bins
            if (RAPBIN(i).lt.RAPBIN(i+1)) then
               WRITE(*,*) "   pseudorapidities from ",RAPBIN(i)," - ",RAPBIN(i+1),
     +              " (D0)        No ET bins: ",NPT(i)
            else                !  happens only for CDF bins
               WRITE(*,*) "   pseudorapidities from ",0.1d0," - ",RAPBIN(i+1),
     +              " (CDF)        No ET bins: ",NPT(i)
            endif
            do j=1,NPT(i)       ! ET Bins
               nbin=nbin+1
               WRITE(*,*) "  ET: ",PTBIN(i,j),"-",PTBIN(i,j+1),

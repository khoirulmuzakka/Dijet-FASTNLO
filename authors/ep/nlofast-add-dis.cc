// last modification 2005/09/06 - TK
...
int main(int argc,void** argv)
{
  // check parameters
  if(argc<3){
    printf("Usage: nlofast-add-dis file1.raw [filex.raw]+ result.txt[.gz]\n");
    return(1);
  }
  // loop over arguments
  int nparts = argc-1;
...
#define READ(n) table.read(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   if(ireaction == kdis){
      disweights.resize (nrap);
      for(int i=0;i<nrap;i++){ // rapidity
         disweights[i].resize(npt[i]);
         for(int j=0;j<npt[i];j++){ // pt
            disweights[i][j].resize(nxtot);
            for(int k=0;k<nxtot;k++){ // x
               disweights[i][j][k].resize(1+nscalevar);
               for(int scalevar=0; scalevar<(nord-1)*nscalevar+1;scalevar++){  // 1 for LO and nscalevar per higher order
                  disweights[i][j][k][scalevar].resize(nsubproc); // three entries in weight
               }
               for(int scalevar=0; scalevar<ncontrib;scalevar++){ //  1 for LO and nscalevar per higher order
                  for(int m=0;m<nsubproc;m++){     //subprocesses
                     READ(disweights[i][j][k][offset+scalevar][m]);
                  }
               }
            }
         }
         ...
         for(int j=0;j<npt[i];j++){ // pt
            for(int k=0;k<nxtot;k++){ // x
               for(int scalevar=0; scalevar<scalevarmax;scalevar++){ //LO & NLO scale variations
                  for(int m=0;m<nsubproc;m++){     //subprocesses
                     READ(bufferdbl);
                     disweights[i][j][k][offset+scalevar][m] += bufferdbl;
                  }
               }
            }
         }
            ...
            for(int k=0;k<nxtot;k++){ // x
               for(int m=0;m<nsubproc;m++){     //subprocesses
                  for(int scalevar=0; scalevar<1+(nordmax-1)*nscalevar;scalevar++){  // 1 for LO and nscalevar per higher order
                     WRITE(disweights[i][j][k][scalevar][m]/nevents[(int)ceil(scalevar/(float)nscalevar)]);
                  }
               }
            }
         }

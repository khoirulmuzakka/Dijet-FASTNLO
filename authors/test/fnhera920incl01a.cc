...
// last modification 2005/07/19 - 6pm(GE) TK
...
void UserDIS::initfunc(unsigned int njet)
{
   ...
   // lowest/highest pT value in sample
   ptlow = 7.0;     
   pthigh.resize(nQ2);
   ...
   // Array used for scale variations
   ...
   for( int j = 0; j < nQ2; j++) {
      ...
      for( int k = 0; k < npt[j]; k++) {
         ...
         xlimit[j][k]= 1.395e-05*Q2+ -3.598e-09*Q2*Q2 + 0.000424*pt -3.035e-06*pt*pt;
	 xsmallest[j][k]=0.999;
	 
         // - Setup the murval and mufval arrays
         murval[j][k]= (0.7*pt + 0.3*pthigh[j][k+1]); // Take as mean pt value 30% right of left bin boundary 
         mufval[j][k]= sqrt(200.); /*(0.7*Q2 + 0.3*Q2high[j][k+1]);*/ // Fixed mur for now 
      }
   }
           ...
           if (Q2bin >=0 ) {
              ...
              if ( ptbin>=0 ) {
                 ...
                 for(int scalevar=0; scalevar<scalevarmax;scalevar++){
                    ...
                    //                    double reweight = 1/sqrt(x) * (1-0.99*x);  
                    //                       wt = wt * reweight*reweight*reweight *389385.730 *alem*alem;
                    ...
                    if (x<xlimit[Q2bin][ptbin]){
                       printf("fnhera920incl01a.cc: Warning: x (%f) < xlimit (%f) at pt=%f GeV Q2=%f GeV2 \n ",
...
void UserDIS::writetable(){
   ...
   //Oalphas
   int Oalphas = npow; // Not generally valid for any observable!!
   WRITE(Oalphas);

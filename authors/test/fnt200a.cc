...
void UserHHC::initfunc(unsigned int)
{
   ...
   for( int j = 0; j < nrap; j++) {
      ...
      for( int k = 0; k < npt[j]; k++) {
         ...
         if (ccorr<1) ccorr = 1;
         xlimit[j][k]= 4.*pt*pt/s * ccorr *0.92;  // w/o 0.9 problem at 1800ET
      ...
      if (pt>ptlow){ // check for low pt first, will fail most of the time
         ...
         if (rapbin >=0 ) {          
            ...
            if ( ptbin>=0 ) {
               ...
               if (xmin<xsmallest[rapbin][ptbin]*0.98){          // factor 0.98 reduces output!
		 xsmallest[rapbin][ptbin] = xmin;
		 //if((itype==amplitude_hhc::lo && nevents> 10000000)||
		 // (itype==amplitude_hhc::nlo && nevents> 1000000)){    // output after 5min

		 if((itype==amplitude_hhc::lo && nevents> 1000000000)||
		    (itype==amplitude_hhc::nlo && nevents> 100000000)){    // output after ~7h

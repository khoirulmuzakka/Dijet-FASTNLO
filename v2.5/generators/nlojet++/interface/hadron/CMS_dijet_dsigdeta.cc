...
private:
   ...
   double deltaPhi12min;         // Minimal deltaPhi12=abs(phi_1-phi_2)
...
void UserHHC::phys_output(const std::basic_string<char>& __file_name, unsigned long __save, bool __txt) {
   ...
   //minimal abs(phi_1-phi_2)
   SteeringPars["deltaPhi12min"] = ftable->TestParameterInSteering("deltaPhi12min");
   ycjjmax = 6.283; // default is 2*pi
...
void inputfunc(unsigned int& nj, unsigned int& nu, unsigned int& nd) {
   ...
   if (!ftable) {
      ...
      ftable = new fastNLOCreate(UsefulNlojetTools::GenConsts(),UsefulNlojetTools::ProcConsts_HHC(),"CMS_dijet.str");

...
private:
   ...
   double ptnjmin;            // Minimal pT of neighbouring jet (default is ptjmin)
...
void UserHHC::phys_output(const std::basic_string<char>& __file_name, unsigned long __save, bool __txt) {
   ...
   // --- fastNLO user: declare and initialize N jet-pair phase space cuts and definitions via steering filejnmin
   // overall lowest pT for neighbouring jets to be considered
   SteeringPars["ptnjmin"] = ftable->TestParameterInSteering("ptnjmin");
   if ( SteeringPars["ptnjmin"] ) {
      ftable->GetParameterFromSteering("ptnjmin",ptnjmin);
   } else {
      ptnjmin = ptjmin;
   }
   // overall minimal distance for neighbouring jets to be considered
   SteeringPars["djlkmin"] = ftable->TestParameterInSteering("djlkmin");
   if ( SteeringPars["djlkmin"] ) {
      ftable->GetParameterFromSteering("djlkmin",djlkmin);
   } else {
      djlkmin = 0;
   }
   // overall maximal distance for neighbouring jets to be considered
   SteeringPars["djlkmax"] = ftable->TestParameterInSteering("djlkmax");
   if ( SteeringPars["djlkmax"] ) {
      ftable->GetParameterFromSteering("djlkmax",djlkmax);
   } else {
      djlkmax = +DBL_MAX;
   }
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp) {
   ...
   // ---- fastNLO v2.2
   // Analyze jet relations in double jet loop (Attention: Includes double counting!)
   ...
   for (unsigned int k = 1; k <= njet; k++) {
      ...
      for (unsigned int l = 1; l <= njet; l++) {
         ...
         double phik  = atan2(pj[k].Y(), pj[k].X());
         double phil  = atan2(pj[l].Y(), pj[l].X());
         double djlk  = abs(dphi(phil,phik));
         if ( ! ldphi ) {
            double dylk = abs(pj[l].rapidity() - pj[k].rapidity());
            djlk = sqrt(djlk*djlk + dylk*dylk);
         }
...
void inputfunc(unsigned int& nj, unsigned int& nu, unsigned int& nd) {
   ...
   if (!ftable) {
      ...
      ftable = new fastNLOCreate(UsefulNlojetTools::GenConsts(),UsefulNlojetTools::ProcConsts_HHC(),"InclusiveNJetPairs.str");

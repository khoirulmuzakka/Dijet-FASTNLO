...
namespace nlo {
  ...
  template<typename _Weight, class _Event, class _EvenTraits> 
  _Weight amplitude<process<_Weight, _Event, _EvenTraits, 1U, 0U> >::
  update_scales(pdf_type *pdf, double mr2, double mf2, double coef) const
  {
    ...
    if(_M_contr == fini) {
      _M_fini.lxr = std::log(mr2/Q2); 
      _M_fini.lxf = std::log(mf2/Q2);
    }
    ...
    _Weight retval = pdf -> pdf(eta, mf2, this->nu(), this->nd());
  ...
  void amplitude<process<_Weight, _Event, _EvenTraits, 1U, 0U> >::amp_fini() const
  {
    ...
    switch(_M_fini.mode) {
    case 0: _M_fini.finix(_M_amp); break;
    case 1: _M_fini.fini1(_M_amp); break;
    }
  }

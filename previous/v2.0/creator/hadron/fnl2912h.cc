//
// fastNLO v2 author code for fnl2912h:
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
  ...
  if (njet > 2) {
    ...
    // minimum pT of triggering jet
    const double pT1min = 50.;
    // minimum pT fraction required for 3rd jet
    const double pT3relmin = 0.25;
    ...
    // Ratio of pT3 to pT1 (pT3rel)
    double pT3rel = pj[3].perp()/pT1;
    
    // --- Further 3-jet phase space cuts?
    if ( m3jmin <= m3j && y3j < y3jmax && pT1min <= pT1 && pT3relmin <= pT3rel ) {
      
      // --- set the renormalization and factorization scale to average 3-jet pT
      double mu = pT123;
      
      // --- identify bin number (dim1,dim2) here (m3j,y3j)
      int obsbin = -1;
...
void UserHHC::inittable(){
  ...
  A2->DimLabel.push_back("|y3j|_max");   // --- fastNLO user: label of 2nd dimension
  A2->IDiffBin.push_back(2);
  ...
  // fnl2912h: divide by bin width in M3j and 2. * |y3j|_max
  ...
  for (int i=0;i<ndim2bins;i++){
	...
	* 2. * (dim2bins[i+1]-dim2bins[i]); // ... e.g. times 2. * d|y3j|_max

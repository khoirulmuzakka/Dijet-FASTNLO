//
// fastNLO v2 author code for fnl2452:
//     CMS LHC Dijet Centrality Ratio Scenario, E_cms = 7 TeV
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
  ...
  if (njet > 1) {
    ...
    const double mjjmin = 489.;
    // maximal pseudo-rapidity of the two leading jets
    ...
    // Pseudo-rapidities of two leading jets
    double y1 = pj[1].prapidity();
    double y2 = pj[2].prapidity();

    // Max and diff for these pseudo-rapidities
    double maxyjj  = max(abs(y1),abs(y2));
    double difyjj  = abs(y2-y1);

    // --- Further dijet phase space cuts?
    if ( mjjmin <= mjj && maxyjj < yjjmax ) {
      
      // --- set the renormalization and factorization scale to average dijet pT
      double mu = (pj[1].perp() + pj[2].perp()) / 2.0;
      
      // --- identify bin number (dim1,dim2) here (mjj,|difyjj|)
      int obsbin = -1;
      for (int j = 0; j < A2->GetNObsBin(); j++) {
	if (A2->LoBin[j][0] <= mjj    && mjj    < A2->UpBin[j][0] && 
	    A2->LoBin[j][1] <= difyjj && difyjj < A2->UpBin[j][1]) {
	  obsbin = j;
	  break;
	}
      }
...
void UserHHC::inittable(){
  ...
  table->GetBlockA1()->SetScenName("fnl2452");  // - fastNLO user: set scenario name
  ...
  A2->ScDescript.push_back("d2sigma-dijet_dMjjd|deta|_[pb_GeV]");
  A2->ScDescript.push_back("CMS_Collaboration");
  A2->ScDescript.push_back("Dijet_Centrality_Ratio");
  A2->ScDescript.push_back("anti-kT_R=0.7");
  A2->ScDescript.push_back("CMS-PAPER-EXO-11-026");
  ...
  A2->DimLabel.push_back("|deta|");   // --- fastNLO user: label of 2nd dimension
  A2->IDiffBin.push_back(2);

  vector <double> bound;
  bound.resize(2);

  // --- fastNLO user: bin definitions - here in Mjj and |deta|
  const int ndim2bins = 2;
  const double dim2bins[ndim2bins+1] = { 0.0, 1.3, 3.0 };
  
  const int ndim1bins[ndim2bins] = { 52, 52 };
  ...
  const double dim0[53] = {  489.,  526.,  565.,  606.,  649.,  693.,  740.,  788.,  838.,  890.,
			     ...
			     3854., 4010., 4171., 4337., 4509., 4686., 4869., 5058., 5235., 5455.,
			     5663., 5877., 6099. };
  for (int i=0; i<ndim2bins; i++) {
  ...
  for (int i=0;i<ndim2bins;i++){
	...
	* 2. * (dim2bins[i+1]-dim2bins[i]); // ... e.g. times d|y|

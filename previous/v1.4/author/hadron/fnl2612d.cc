//
// fastNLO author code for fnl2612:
...
void UserHHC::initfunc(unsigned int)
{
   ...
   // # of bins
   nrap = 9;
   double rapb[10] = { 250., 350., 500., 650., 850., 1100.,
		       1400., 1800., 2200., 7000.};
   ...
   int nptb[9] = {14, 14, 14, 14, 14, 14, 14, 14, 14};
   npt = new int[nrap];
   for (unsigned int i=0; i<nrap/(iref+1); i++) {
     npt[i] = nptb[i]; 
   }
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
  ...
  //iproc
   ...
   //JetResol1
   double JetResol1 = 0.7; // distance parameter R
   WRITE(JetResol1);

//
// fastNLO author code for fnl2722 (differential in HT):
...
void UserHHC::initfunc(unsigned int)
{
   ...
   // Here: One bin in rapidity (no normalization to bin width!):
   //       1) Only 3+ jets in |y| < 2.5
   // # of bins
   nrap = 1;
   double rapb[2] = { 0.0, 2.5 };
   ...
   for( int j = 0; j < nrap; j++) {
	 ...
	 xlimit[j][k] = xlimit[j][k]*0.95; // small safety factor -> E-scheme
         ...
         murval[j][k]= (0.55*pt + 0.45*pthigh[j][k+1]/2.); 
         mufval[j][k]= (0.55*pt + 0.45*pthigh[j][k+1]/2.); 
      }
   }
...
// Initialize pointers to the jets, check minimal jet pT and maximal |y,eta|
   ...
   if (njet >= 2) {
     ...
     if ( njc > 1 && ht > htmin ) {
       ...
       // --- determine y(=rap) and pT(=HT) bin (njc = 3+) 
	 ...
	 //DEBUGEND
            ...
            if ( ptbin>=0 ) {
               ...
               if (xmin<xlimit[rapbin][ptbin]){
                  printf("Warning: xmin (%f) < xlimit (%f) at pt=%f GeV y=%i \n ",
                         xmin,xlimit[rapbin][ptbin],ht,njc);
                  exit(1);
               }

//
// fastNLO author code for fnl2272e:
//     CMS LHC Forward-Central Jets Scenario, E_cms = 7 TeV
...
void UserHHC::initfunc(unsigned int)
{
  ...
  // Here: Jet pseudo-rapidity
  // # of bins
  nrap = 3;
  double rapb[4] = { 0.0, 2.8, 3.2, 4.7 };
  ...
  // Here:  Jet pT
  // 
  // First pt bin to use per irap bin of first dimension
  int npti[3] = {1,0,0};
  // Last pt bin to use per irap bin of first dimension
  int nptf[3] = {5,0,5};
  // # of bins npt per irap bin of first dimension
  int nptb[3] = {0,0,0};
  for (unsigned int i=0; i<nrap/(iref+1); i++) {
    nptb[i] = nptf[i] - npti[i] + 1;
  }
  npt = new int[nrap];
  for (unsigned int i=0; i<nrap/(iref+1); i++) {
    npt[i] = nptb[i]; 
  ...
  // Array for bin boundaries
  double ptb[7] = { 35., 45., 57., 72., 90., 120., 150. };
  for (unsigned int i=0; i<nrap/(iref+1); i++) {
    for (int j=0; j<npt[i]+1; j++) { 
      pthigh[i][j] = ptb[j+npti[i]];
    }
  }
  // In reference mode: Copy high end of bin boundaries
  if ( iref==1 ) {
  ...
  cout << "    Scenario fnl2272e:" << endl;
  cout << "      CMS LHC Forward-Central Jets Scenario, E_cms = 7 TeV," << endl;
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
  ...
  // minimal pT difference of central jet wrt the forward jet
  double dptctof = 10.;
  // lowest pT for forward jets
  double ptflow = pthigh[nrap-1][0];
  // lowest pT for all
  double ptlow = ptflow;
  // highest pT still binned
  double pthig = max(pthigh[0][npt[0]],pthigh[2][npt[2]]);
  // upper limit in (pseudo-)rapidity for central jets
  double yjcmax = raphigh[1];
  // lower limit in (pseudo-)rapidity for forward jets
  double yjfmin = raphigh[2];
  // highest (pseudo-)rapidity for jets to be considered
  double yjmax = raphigh[nrap];

  //DEBUG
  //     cout << "-------------------- Next event --------------------" << endl; 
  //     cout << "dptctof, ptflow, ptlow, pthig = " << dptctof << ", " << ptflow << ", " << ptlow << ", " << pthig << endl;
  //     cout << "yjcmax, yjfmin, yjmax = " << yjcmax << ", " << yjfmin << ", " << yjmax << endl;
  //DEBUGEND
  ...
  if (nj > 0) {
	...
	if ( pt4 > pt1 ) {
      ...
      // Define central jet as highest pT one at central rapidity and
      // define forward jet as highest pT one at forward rapidity
      int ijc = 0;
      int ijf = 0;
      if ( abs(pj[ij1].rapidity()) < yjcmax && ijc == 0 ) {
	ijc = ij1;
      } else if ( yjfmin < abs(pj[ij1].rapidity()) && abs(pj[ij1].rapidity()) < yjmax && ijf == 0 ) {
	ijf = ij1;
      }
      if ( abs(pj[ij2].rapidity()) < yjcmax && ijc == 0 ) {
	ijc = ij2;
      } else if ( yjfmin < abs(pj[ij2].rapidity()) && abs(pj[ij2].rapidity()) < yjmax && ijf == 0 ) {
	ijf = ij2;
      }
      if ( njet > 2 ) {
	if ( abs(pj[ij3].rapidity()) < yjcmax && ijc == 0 ) {
	  ijc = ij3;
	} else if ( yjfmin < abs(pj[ij3].rapidity()) && abs(pj[ij3].rapidity()) < yjmax && ijf == 0 ) {
	  ijf = ij3;
	}
      }
      if ( njet > 3 ) {
	if ( abs(pj[ij4].rapidity()) < yjcmax && ijc == 0 ) {
	  ijc = ij4;
	} else if ( yjfmin < abs(pj[ij4].rapidity()) && abs(pj[ij4].rapidity()) < yjmax && ijf == 0 ) {
	  ijf = ij4;
	}
      }
      ...
      if ( ijc > 0 ) {
	ptc = pj[ijc].perp();
	yc = abs(pj[ijc].rapidity());
      }
      if ( ijf > 0 ) {
	ptf = pj[ijf].perp();
	yf = abs(pj[ijf].rapidity());
      }
      
      //DEBUG
      //       cout << "Selected dijets before cuts: " << endl;
      //       cout << "ijc, ptc, yc: " << ijc << ", " << ptc << ", " << yc << endl;
      //       cout << "ijf, ptf, yf: " << ijf << ", " << ptf << ", " << yf << endl;
      //DEBUGEND

      // Determine maximal pT
      double ptmax  = max(ptc,ptf);
    
      if ( ptmax < pthig && ptflow < ptf && ptf + dptctof < ptc && yc < yjcmax && yjfmin < yf && yf < yjmax ) {

	// Analyze leading jets in jet loop
	for (int i = 1; i <= nj; i++) {
	  // Only the selected dijets
	  if (i==ijc || i==ijf) {
	    ...
	    //DEBUG
	    // 	    cout << "Selected jet after cuts: " << endl;
	    // 	    cout << "i, pt, y: " << i << ", " << pt << ", " << rap << endl;
	    //DEBUGEND
	  ...
	  } // - end: one of the two leading jets
	} // - end: jet loop 
      } // - end: central-forward conditions
    } // - end: nj>=2
  } // - end: nj>0
}
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
  ...
  table << "Forward_Central_Dijets" << endl;
  ...
  // absolute order
  int npow = nord+2; // -> here "+2" for 3-jet xsect
  WRITE(npow);

//
// fastNLO author code for fnl2272d:
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
  ...
  if (nj > 0) {
	...
	if ( pt4 > pt1 ) {
      ...
      // For the two leading jets find more central and more forward one
      int ijc = ij1;
      int ijf = ij2;
      double ptc = pt1;
      double ptf = pt2;
      if ( abs(pj[ij2].rapidity()) < abs(pj[ij1].rapidity()) ) {
	ijc = ij2;
	ijf = ij1;
	ptc = pt2;
	ptf = pt1;
      }
      double yc = abs(pj[ijc].rapidity());
      double yf = abs(pj[ijf].rapidity());

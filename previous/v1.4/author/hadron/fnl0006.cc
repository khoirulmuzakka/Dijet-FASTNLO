//
// fastNLO author code for fnl0006
//         LHC dijet Chi distribution
//
// 
// last modification
...
void UserHHC::initfunc(unsigned int)
{
   ...
   //Set up binning  
   nrap = 7;  //             // No. of bins in rapidity -> here: Mass
   ...
   //Define binning in pt -> here:  Chi bins  1.0-16.0: 14 bins
   ...
   ptlow = 60.;          // absolute lowest pT-cut
   ...
   for( int j = 0; j < nrap; j++) { 
	 ...
	 if (j==0) xlimit[j][k] = xlimit[j][k] * 1.90;
	 if (j==1) xlimit[j][k] = xlimit[j][k] * 1.50;
	 if (j==2) xlimit[j][k] = xlimit[j][k] * 1.30;
	 if (j==3) xlimit[j][k] = xlimit[j][k] * 1.20;
	 if (j==4) xlimit[j][k] = xlimit[j][k] * 1.10;
	 if (j==5) xlimit[j][k] = xlimit[j][k] * 1.00;
	 if (j==6) xlimit[j][k] = xlimit[j][k] * 1.00;
	 ...
	 cout<<"pT  "<<pthi[j][k]<<"  "<<ptlo[j][k]<<" "<<dpt<<"  "<<
	   (ptlo[j][k]+dpt*0.19)<<" "<<(pthi[j][k]-dpt*0.4)
         ...
         murval[j][k][0] = ptlo[j][k] +dpt*0.19; 
         murval[j][k][1] = pthi[j][k] -dpt*0.4;
         
         if (j==6) { // special definition in last - large - mass bin
	   murval[j][k][0] = ptlo[j][k] +dpt*0.03;
	   murval[j][k][1] = pthi[j][k] -dpt*0.82;   
	 }
   ...
   cout<<"         *** scenario fnl0006 ***"<<endl;
   cout<<"              (Chi distrib)"<<endl;
   cout<<" "<<endl;
   ...
   if (nj>=2) {
     ...
     */
     ...
     cout<<dijmass<<" "<<dijchi<<" "<<yboost<<"   "<<pj[ij1].rapidity()<<
       " "<<pj[ij2].rapidity()<<"   "<<
       pj[ij1].perp()<<" "<<pj[ij2].perp()<<endl;
     */

     // --- cuts
     if (pt1>ptlow && dijmass > 2000.0 && dijchi<16.0 && yboost<1.0) {
	 ...
	 if ( ptbin>=0 ) {
	   ...
	   if (xmin<xsmallest[rapbin][ptbin]*0.98){  // 0.98 reduces output
	     ...
	     if((itype==amplitude_hhc::lo && nevents> 700000000)|| // 2.5h 
	       ...
	       //if((itype==amplitude_hhc::lo && nevents> 10000000)|| //10min 
	       //(itype==amplitude_hhc::nlo && nevents> 1000000)){  // 
	       cout<<" "<<endl;

...
*                            
* fastNLO user code  
*         -> special version for scenario fnt1006 which is
*            the ratio of two cross sections (fnt1006a, fnt1006b) 
*
...
*******************************************************************
      SUBROUTINE FT1006CC(FILENAME1, FILENAME2, 
     +     XMUR, XMUF, IPRINTFLAG, XSECT)
*-----------------------------------------------------------------
* fastNLO user code - main routine 
*
* input:
*   FILENAME1    name of 1st input table (546GeV)
...
*-----------------------------------------------------------------
      ...
      DOUBLE PRECISION xmur, xmuf, array1(11,3), array2(11,3),xsect(11,3)


      call FT1006aCC(FILENAME1, xmur,xmuf, 0, array1)
      call FT1006bCC(FILENAME2, xmur,xmuf, 0, array2)
     ...
     +        'sigma(546GeV)/sigma(1800GeV) vs. xT'
         write(*,*) '--------  muf/mu0= ',xmuf,'    mur/mu0= ',xmur
         write(*,*) '--              from 0.1 to 0.7 in eta'
...
*******************************************************************
      SUBROUTINE FT1006ACC(FILENAME, XMUR, XMUF, IPRINTFLAG, XSECT)
*-----------------------------------------------------------------
* fastNLO user code - main routine 
*
* input:
...
c - now get the PDFs ...
      call FT1006AGP(mufscale(ixmuf))
c - ... and multiply with perturbative coefficients 
      call FT1006AMT(xmur,ixmuf)
...
c - print results - if requested
      if (IPRINTFLAG.eq.1) call FT1006APR(xmur,IXMUF)
...
*******************************************************************
      SUBROUTINE FT1006AMT(xmur,ixmuf)
*-----------------------------------------------------------------
* MW 08/26/2005
*
...
*******************************************************************
      SUBROUTINE FT1006AGP(muffactor)
*-----------------------------------------------------------------
* MW 08/26/2005
*
...
c - now fill main PDF array - compute different lin. comb for diff sub-proc
               ...
               do k=1,NXTOT
                  ...
                  do l=1,nx2limit
                     nx=nx+1
                     call ft1006apl(ireaction,k,l,XPDF,H)
                     do m=1,Nsubproc
                        pdf(nbin,nx,m,p) = H(m)
                     enddo
                  enddo
...
*******************************************************************
      SUBROUTINE FT1006APL(ireact,i,j,XPDF,H)
* ---------------------------------------------------------------
* MW 03/27/06
...
*******************************************************************
      ...
      SUBROUTINE FT1006APR(XMUR,IMUFFLAG)
*-----------------------------------------------------------------
...
*******************************************************************
      SUBROUTINE FT1006BCC(FILENAME, XMUR, XMUF, IPRINTFLAG, XSECT)
*-----------------------------------------------------------------
* fastNLO user code - main routine 
*
* input:
...
c - now get the PDFs ...
      call FT1006BGP(mufscale(ixmuf))
c - ... and multiply with perturbative coefficients 
      call FT1006BMT(xmur,ixmuf)
...
c - print results - if requested
      if (IPRINTFLAG.eq.1) call FT1006BPR(xmur,IXMUF)
...
*******************************************************************
      SUBROUTINE FT1006BMT(xmur,ixmuf)
*-----------------------------------------------------------------
* MW 08/26/2005
*
* multiply the PDFs and the coefficients for a single scale setting
...
*******************************************************************
      SUBROUTINE FT1006BGP(muffactor)
*-----------------------------------------------------------------
* MW 08/26/2005
*
* read the PDFs in all (rap,pT) bins - at all xmax, xmin bins
...
c - now fill main PDF array - compute different lin. comb for diff sub-proc
               ...
               do k=1,NXTOT
                  ...
                  do l=1,nx2limit
                     nx=nx+1
                     call ft1006bpl(ireaction,k,l,XPDF,H)
                     do m=1,Nsubproc
                        pdf(nbin,nx,m,p) = H(m)
                     enddo
                  enddo
...
*******************************************************************
      SUBROUTINE FT1006BPL(ireact,i,j,XPDF,H)
* ---------------------------------------------------------------
* MW 03/27/06
* compute PDF linear combinations - for different reactions
*
...
*******************************************************************
      ...
      SUBROUTINE FT1006BPR(XMUR,IMUFFLAG)
*-----------------------------------------------------------------
* m. Wobisch  - print fastNLO cross section results
*
* input:

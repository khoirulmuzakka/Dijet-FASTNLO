...
class NNLOJETHistogram():
    ...
    def _read_dat(self, **kwargs):
        ...
        if self._labels is None:
            print("WARNING: couldn't find a labels line {}".format(self.filename))
            return
        ...
        if columns is not None:
            ...
            for col in columns:
                if re.match(r'.*_Err\b', col, re.IGNORECASE):
                    raise ValueError('columns must noch contain *_Err column names!')
        ...
        for line in fh.readlines():
            ...
            if re.match(comment_pattern, line):
                ...
                if re.match(r'^\s*' + _comment_prefix + 'overflow', line, re.IGNORECASE):
                    ...
                    nrows += 1
                    if len(data) != ncols:
                        raise ValueError('ncols mismatch in overflow: {} != {}'.format(len(data), ncols))
            #----- read data
            else:
                data = line.split()
                if self.nx > 0:
                ...
                nrows += 1
                if len(data) != ncols:
                    raise ValueError('ncols mismatch in data: {} != {}'.format(len(data), ncols))
        fh.close()
            ...
            for i_row in range(nrows):
                ...
                if data_xval[i_row][0] >= rebin[i_rebin] and data_xval[i_row][2] <= rebin[i_rebin+1]:
                    for i_col in range(len(rebin_row_val)):
                        ...
                        rebin_row_val[i_col] += data_yval[i_row][i_col]*delta
                        rebin_row_err[i_col] += (data_yerr[i_row][i_col]*delta)**2
                # finalise
                if i_row == nrows-1 or data_xval[i_row+1][0] >= rebin[i_rebin+1]:
                    ...
                    for i_col in range(len(rebin_row_val)):
                        rebin_row_val[i_col] = rebin_row_val[i_col]/delta
                        rebin_row_err[i_col] = math.sqrt(rebin_row_err[i_col])/delta
    ...
    def __str__(self):
        ...
        for irow, yarr in enumerate(self._yval):
            ...
            for icol, y in enumerate(yarr):
                line += '{:E} {:E} '.format(y, self._yerr[irow, icol])
            lines.append(line)
        return '\n'.join(lines)
...
#         return wgts_out
    ...
    def _merge_bin(self, i_row, i_col, skip_wgt=False):
        ...
        for i_file in range(n_files):
            ...
            yerr += self._neval[i_file] * self._yval[i_row, i_col, i_file]**2 + (self._neval[i_file] * self._yerr[i_row, i_col, i_file])**2
    ...
    def mask_outliers(self, thresh=3.5, maxfrac=0.1):
        if thresh <= 0.:
    ...
    def optimise_k(self, **kwargs):
        ...
        # set up defult values for steps if one parameter is set
        if nsteps is not None and maxdev_steps is None:
            maxdev_steps = 1.
        if maxdev_steps is not None and nsteps is None:
            nsteps = 2
        ...
        for i_row in range(n_rows):
            for i_col in range(n_cols):
                ...
                while n_unmasked > 1:
                    ...
                    # check for `maxdev_steps` * sigma compatibility to last `nsteps` steps
                    if maxdev_steps is not None and len(history) >= nsteps:
                        term_cond2 = True
                        for istep in range(-1,-nsteps-1,-1):
                            delta = abs(yval_wgt - history[istep][0])
                            sigma = math.sqrt(yerr_wgt**2 + history[istep][1]**2)

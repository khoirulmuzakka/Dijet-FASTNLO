...
def main():
    ...
    logging.basicConfig(filename=config.get('Paths', 'out_dir') + '/combine.log', filemode='w', level=logging.INFO)
...
#    obs_set = set( map(lambda it: it.split('>')[0].strip(), obs_list) )
...
#        if rebin is not None:
#            rebin = ast.literal_eval(rebin)
#            print("rebinning {} > {}...".format(obs_in, obs_out))
#        else:
#            print("renaming {} > {}...".format(obs_in, obs_out))
#
#        # combine the different observables
#        for pt in config.options('Parts'):
#            mappt = config.get('Parts', pt)
#            if mappt is None:
#                mappt = pt
#            infile = outdir_parts + '/' + mappt + '.' + obs_in + '.dat'
#            outfile = outdir_parts + '/' + mappt + '.' + obs_out + '.dat'
#            hist = NNLOJETHistogram(nx=nx, filename=infile, weights=qweights, recursive_weights=qweights, rebin=rebin)
        ...
        for (obs_line, obs_in, obs_out, nx) in obs_generator(obs_list):
            ...
            for mrg in config.options('Merge'):
                ...
                if int("|" in parts) + int("&" in parts) + int("+" in parts) > 1:
                    ...
                    ptfile = outdir_parts + '/' + parts + '.' + obs_out + '.dat'
                    hist = NNLOJETHistogram(nx=nx, filename=ptfile, weights=qweights)
                    hist.write_to_file(outfile)

...
for i in range(nscl):
    ...
    ax.set_ylabel(r'$\bf\left|d\sigma/dp_T\right|$ [pb/GeV]',
                  horizontalalignment='right', x=1.0, verticalalignment='top', y=1.0, labelpad=20)
    axr.set_ylabel('Ratio', horizontalalignment='center',
                   verticalalignment='center', labelpad=20)
    ax.set_xticklabels([])

    if seed == '' or seed == '_':
        ax.set_title(r'Merged grid: {} {} {} for scale choice {}'.format(
            proc, jobn, obsv, sclnam[i]), fontweight=titwgt, y=1.05)
    else:
        ax.set_title(r'Single grid: {} {} {} for scale choice {}'.format(
            proc, jobn, obsv, sclnam[i]), fontweight=titwgt, y=1.05)
...
# Booleans for + and - x sections
    ...
    if len(x[xmp]):
        abs1p = ax.errorbar(x[xmp], +xs_nnlo[i][xmp], xerr=dx[xmp], capsize=0, yerr=dxs_nnlo[i]
                            [xmp], marker=',', linestyle='none', label=r'NNLOJET $\bf\pm\Delta_{stat}$', color=col1)
        axhandles.append(abs1p)
    if len(x[xmn]):
        abs1n = ax.errorbar(x[xmn], -xs_nnlo[i][xmn], xerr=dx[xmn], capsize=0, yerr=dxs_nnlo[i]
                            [xmn], marker=',', linestyle='none', label=r'NNLOJET', color=col1)
#        axhandles.append(abs1n)
    ...
    if len(x[xmp]):
        abs2p = ax.errorbar(x[xmp], +xs_fnll[i][xmp], marker='d', linestyle='none',
                            label=r'APPLfast grid ($\bf\sigma>0$)', color=col2)
        axhandles.append(abs2p)
    if len(x[xmn]):
        abs2n = ax.errorbar(x[xmn], -xs_fnll[i][xmn], marker='d', linestyle='none',
                            label=r'APPLfast grid ($\bf\sigma<0$)', color=col3, markerfacecolor=col3)
    ...
    rnn = axr.errorbar(x, r_nn2nn[i], marker=',',
                       linestyle='none', label='', color=col1)
    ...
    rfn = axr.errorbar(x[xmn], r_fl2nn[i][xmn], marker='d',
                       linestyle='none', label='', color=col3, markerfacecolor=col3)
    ...
    if xaxe == 'bins':
        if args['outputfilename'] is None:
            fignam = proc+'.'+jobn+'.'+obsv+'.' + \
                'scale-no-'+str(i+1)+'.absolute-index'+'.png'
        else:
            fignam = args['outputfilename']+'.scale-no-' + \
                str(i+1)+'.absolute-index'+'.png'
    else:
        if args['outputfilename'] is None:
            fignam = proc+'.'+jobn+'.'+obsv+'.' + \
                'scale-no-'+str(i+1)+'.absolute-obs'+'.png'
        else:
            fignam = args['outputfilename']+'.scale-no-' + \
                str(i+1)+'.absolute-index'+'.png'

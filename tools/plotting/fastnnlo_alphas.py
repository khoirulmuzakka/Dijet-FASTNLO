...
def plotting(alphas_list, xs_chosen, order_list, x_axis, xlabel, tablename, pdfset, xmin, xmax, given_filename):
	...
	if len(alphas_list)==1:
		...
		print '[fastnnlo_alphas]: Max. possible: 5. Number of given alpha_s values: %s'%len(alphas_list)
		sys.exit()
	...
	for order_item in order_list: #producing one plot per order (comparing all given alpha_s values)
		fig = plt.figure(figsize=(7,7))
		ax1 = plt.subplot(gs[:-1,:])
		...
		for alpha, shift in zip(alphas_list, shift_list): #go through all given values for alpha_s
			c = next(color)
			ax1.errorbar(x_axis*shift, xs_chosen[alphas_index, order_index, :], elinewidth=1, linewidth=0.0, ms=4, color=c, fmt='.', label=r'$\alpha_s = %s$'%alpha) #uncertainties?
		...
		ax1.set_ylabel(r'XS for different $\alpha_s$', fontsize=12, rotation=90)
		ax1.legend(fontsize=10, numpoints=1)
		ax1.text(0.04, 0.08, '%s' %order_item, fontsize=12, horizontalalignment='left', verticalalignment='bottom', transform=ax1.transAxes)
		ax1.text(0.04, 0.02, '%s' %pdfset, fontsize=12, horizontalalignment='left', verticalalignment='bottom', transform=ax1.transAxes)
		...
		ax2.set_ylabel(r'Ratio $\alpha_{s, i}$ to $\alpha_{s, 0}$', fontsize=12, rotation=90)
		ax2.text(0.02, 0.92, r'Reference $\alpha_s$: %s'%alphas_list[0], horizontalalignment='left', verticalalignment='top', transform=ax2.transAxes)
		ax2.axhline(y=1, xmin=0, xmax=1, linewidth=0.4, color='k', linestyle='dotted')
		
		plt.grid(b=True, which='both', axis='both', color='k', linestyle='steps--', alpha=0.2)
		...
		if given_filename is not None:
			filename = '%s.alphas-%s.%s' %(given_filename, order_item, alphanames[1:])
		else:
			filename = '%s.alphas-%s.%s' %(tablename, order_item, alphanames[1:])

		fig.savefig('%s.png' %filename)
		print '[fastnnlo_alphas]: %s plot saved as: %s.png' %(order_item, filename)
		plt.close()
...
def main():
	...
	if args['order'] is None:
		...
		iordmin = min(iorders)
		iordmax = max(iorders)
		print '[fastnnlo_alphas]: Evaluate table in the following order(s): ', args['order']
	...
	fnlo = fastnlo.fastNLOAlphas(args['table'], pdfset, args['member']) #use fastNLOALphas() instead of fastNLOLHAPDF() in order to change alpha_s
	...
	if args['order'] is not None:
		...
		for i in [0, 1, 2]: #assuming we have no NNNLO tables yet, could of course be kept more general
			...
			if _order_existence[i]==False:
				...
				if verb: print '[fastnnlo_alphas]: Given table contains %s. \n'%_order_to_text[i]
	...
	xs_list = [] #will contain for each given alpha_s value the corresponding cross section in requested (or available) orders
	...
	if verb:
		...
		print '[fastnnlo_alphas]: For each alpha_s %s xs_chosen contains XS corresponding to %s.'%(alphas_list, order_list)
		print xs_chosen, '\n \n'
	...
	plotting(alphas_list, xs_chosen, order_list, x_axis, xlabel, tablename, pdfset, xmin, xmax, given_filename)

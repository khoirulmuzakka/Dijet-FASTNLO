...
# add arguments (one .dat, three .tab.gz and two .log files)
parser.add_argument('-d', '--datfile', default='file.dat', required=True,
                    help='NNLOJET .dat file for evaluation.')
parser.add_argument('-t', '--tables', default='table.tab', required=True, nargs=3,
                    help='Need one NNLOJET fastNLO table and two NLOJet++ fastNLO tables.')
parser.add_argument('-l', '--logfiles', default='file.log', required=True, nargs=2,
                    help='Two NLOJet++ fastNLO .log files, i.e. "statlogs" for uncertainty.')

parser.add_argument('-p', '--pdfset', default='CT14nnlo', required=False, type=str,
                    help='Optional choice of PDF Set for evaluation. '
                    'Default: CT14nnlo.')
...
for table in args['tables']:
    ...
    if (len(tab.split(".")) > 3):
        nnlotab = table
    elif (len(tab.split(".")) == 3):
        if 'max' in tab:
            nlotab2 = table
        else:
            nlotab1 = table
    else:
        "Check given tables. Could not match them to respective variable."
print "NNLOJET fastNLO table is: ", nnlotab
print "NLOJET++ tables are: ", nlotab1, "and ", nlotab2, '\n'
# statlogs:
...
nnlotab_base = os.path.basename(nnlotab)
nnlotab_args = nnlotab_base.split(".")
print 'nnlotab_args: ', nnlotab_args
...
# arguments from NNLOJET fastNLO table (= nnlotab)
if len(nnlotab_args) == 6:
    proc, jobn, kinn, obsv, ext1, ext2 = nnlotab_args
    print 'proc: ', proc
    print 'jobn: ', jobn
    print 'kinn: ', kinn
    print 'obsv: ', obsv, '\n'
    print '\n'
elif len(nnlotab_args) == 5:
    proc, jobn, obsv, ext1, ext2 = nnlotab_args
    print 'proc: ', proc
    print 'jobn: ', jobn
    print 'obsv: ', obsv
    print '\n'
...
xs_fnl1 = []
xs_fnl2 = []
"""#already assigned earlier when handling the input
...
""" #shall we leave this kind of automatic? right now: have to give it as input...
if 'NLO' in order:
...
r_fnnl1 = np.divide(xs_fnnl1, xs_nnlo, out=np.ones_like(
    xs_fnnl1), where=xs_nnlo != 0)
r_fnnl2 = np.divide(xs_fnnl2, xs_nnlo, out=np.ones_like(
    xs_fnnl2), where=xs_nnlo != 0)
dr_fnnl1 = np.multiply(r_fnnl1, dst_nnlo)
dr_fnnl2 = np.multiply(r_fnnl2, dst_nnlo)
r_fnl1 = np.divide(xs_fnl1, xs_nnlo, out=np.ones_like(
    xs_fnl1), where=xs_nnlo != 0)
r_fnl2 = np.divide(xs_fnl2, xs_nnlo, out=np.ones_like(
    xs_fnl2), where=xs_nnlo != 0)
...
fig = plt.figure()
ax = fig.gca()
plt.title(r'Ratios to NNLOJET: {} {} {}'.format(
    proc, jobn, obsv), fontweight=titwgt)
plt.xlabel('Observable bin index', horizontalalignment='right',
           x=1.0, verticalalignment='top', y=1.0)
plt.ylabel('Ratio to NNLOJET', horizontalalignment='right',
...
ratio1 = plt.errorbar(x, r_fnnl1, yerr=0.*dr_fnnl1, marker='s', linestyle='none',
                      label=r'NNLOJET grid, $\mu_r=\mu_f=p_{\rm T,jet}$', color='blue')
ratio2 = plt.errorbar(x, r_fnnl2, yerr=0.*dr_fnnl2, marker='^', linestyle='none',
                      label=r'NNLOJET grid, $\mu_r=\mu_f=p_{\rm T,max}$', color='orange')
# Show quadratically added statistical uncertainties for NLOJet++ / NNLOJET
ratio3 = plt.errorbar(x, r_fnl1, yerr=dr_fnl1, marker='o', linestyle='none',
                      label=r'NLOJet++ grid, $\mu_r=\mu_f=p_{\rm T,jet}$', color='magenta')
ratio4 = plt.errorbar(x, r_fnl2, yerr=dr_fnl2, marker='v', linestyle='none',
                      label=r'NLOJet++ grid, $\mu_r=\mu_f=p_{\rm T,max}$', color='green')
...
legend = ax.legend(handles, labels, title=r'APPLfast / NNLOJET',
                   loc='upper left', numpoints=1, handlelength=0)
legend.get_title().set_fontsize(limfs)

if args['outputfilename'] is None:
    fignam = proc+'.'+jobn+'.'+obsv+'.'+'nlocomp'+'.png'
else:
    fignam = args['outputfilename']+'.nlocomp.png'
plt.savefig(fignam)

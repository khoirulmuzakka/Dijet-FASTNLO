...
for arg in args['datfiles']:
    ...
    # new datfile name-format with <proc>.<ord>....dat
    pre = arg0.split('.')[1]
...
kn_nnlo = np.divide(xs_nnlo, xs_nlo, out=np.ones_like(
    xs_nnlo), where=xs_nlo != 0)
...
dst_nlo = np.divide(dxs_nlo, xs_nlo, out=np.ones_like(
    dxs_nlo), where=xs_nlo != 0)
...
ax.set_ylabel(r'$\bf d\sigma/dp_T$ [pb/GeV]', horizontalalignment='right',
              x=1.0, verticalalignment='top', y=1.0, labelpad=20)
axr.set_ylabel('K factor', horizontalalignment='center',
               verticalalignment='center', labelpad=20)
ax.set_xticklabels([])

ax.set_title(r'Cross sections and K factors: {} {} {}'.format(
    proc, jobn, obsv), fontweight=titwgt, y=1.05)
...
abslo = ax.errorbar(x, xs_lo, xerr=dx, capsize=0, yerr=dxs_lo, marker=',',
                    linestyle='none', label=r'LO $\bf\pm\Delta_{stat}$', color=col1)
axhandles.append(abslo)
absnlo = ax.errorbar(x, xs_nlo, xerr=dx, capsize=0, yerr=dxs_nlo, marker='s',
                     linestyle='none', label=r'NLO $\bf\pm\Delta_{stat}$', color=col3)
axhandles.append(absnlo)
absnnlo = ax.errorbar(x, xs_nnlo, xerr=dx, capsize=0, yerr=dxs_nnlo, marker='o', linestyle='none',
                      label=r'NNLO $\bf\pm\Delta_{stat}$', color=col2, mfc='none', mec=col2, mew=3)
...
rkn = axr.errorbar(x, knlo, yerr=dk_nlo, marker='s',
                   linestyle='none', label='', color=col3)
rknn = axr.errorbar(x, knnlo, yerr=dk_nnlo, marker='o',
                    linestyle='none', label='', color=col2)
axrhandles = [rkn, rknn]

axlabels = [h.get_label() for h in axhandles]
axrlabels = [h.get_label() for h in axrhandles]

legend = ax.legend(axhandles, axlabels, title=r'NNLOJET inclusive jet $\bf p_T$',

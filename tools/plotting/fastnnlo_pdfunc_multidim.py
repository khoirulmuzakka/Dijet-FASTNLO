...
def dimensionhandler(fnlo, verb):
	...
	binningdim = ndim-1 #number of dimensions minus 1 (starting indexing at 0)
	...
	total_binbounds = np.array(fnlo.GetObsBinsBounds(binningdim)) #list in which bins show up repeatedly if ndim>1
	...
	x_axis_list=[]
	tmp_len_list=[] #contains the index of the "last lower bin bound" for each plot to be produced
	while (len(tmp_binbounds)>0):	
		for i in range(1, len(tmp_binbounds[:, 0])): #go through all lower binbounds
			if (tmp_binbounds[i, 0]<=tmp_binbounds[(i-1), 0]): # check: when does "next x-axis" start? // when do lower bin bounds get smaller again 
				#only first time adding i-1 (because first index in total_binbounds etc. is 0) 
				#...otherwise add i
				if nplot==0:
					tmp_len += (i-1)
				else:
					tmp_len += i # x-axis of plot goes up to index(!) tmp_len (later, this is necessary for plotting routine)
			...
			elif (i==(len(tmp_binbounds[:,0])-1)): #happens if the current tmp_binbounds is the last array (= lower bin bound does not decrease anymore.)
				#append last plot's bin bounds:
				plots_binbounds_list.append(tmp_binbounds)
				tmp_len += (i+1) #because of elif-condition here, index had no chance to rise one step further
				tmp_len_list.append(tmp_len)
				nplot += 1
				tmp_binbounds=[]
				tmp_binbounds=np.array(tmp_binbounds)

	all_bin_bounds=np.array(plots_binbounds_list) 	#this is a numpy array containing lists of (usually) different length.
	...
	for n in range(0, nplot):
		...
		x_axis_list.append((all_bin_bounds[n].T[0]+all_bin_bounds[n].T[1])/2.) #this is a list which contains one list of bincenters for each plot
		...
		x_errors_tmp = np.array([-1*(all_bin_bounds[n].T[0]-x_axis_list[n]), all_bin_bounds[n].T[1]-x_axis_list[n]])
...
def plotting(pdfsets, order_list, xs_chosen, abs_pdf_unc, rel_pdf_unc, xlabel, tablename, given_filename, x_axis, xmin, xmax, nameDimO, nameDimM, nameDimI, dimlabels, plotnr):
	...
	if (len(pdfsets)>1): #one plot contains all chosen pdf sets for one specific order + summary plot if 3 orders in order_list
		...
		for order_item in order_list: #producing one plot per order (comparing all the given pdfs)
			...
			ax1.text(0.06, 0.12, '%s' %nameDimO, fontsize=12, horizontalalignment='left', verticalalignment='bottom', transform=ax1.transAxes) 
			ax1.text(0.06, 0.18, '%s' %nameDimM, fontsize=12, horizontalalignment='left', verticalalignment='bottom', transform=ax1.transAxes)
			...
			if given_filename is not None:
				filename = '%s.pdfunc-%s.plot%s.%s.%s' %(given_filename, order_item, plotnr, dimspeci, pdfnames[1:])
			else:
				filename = '%s.pdfunc-%s.plot%s.%s%s' %(tablename, order_item, plotnr, dimspeci, pdfnames[1:])
		...
		if len(order_list)==3: #could adjust this to arbitrary length of order
			...
			for ax, order in zip([ax0, ax1, ax2], order_list): #here it can happen, that NNLO is plotted in ax0, depending on content of order_list
				...
				ax.text(0.06, 0.18, '%s' %nameDimO, fontsize=12, horizontalalignment='left', verticalalignment='bottom', transform=ax1.transAxes) 
				ax.text(0.06, 0.24, '%s' %nameDimM, fontsize=12, horizontalalignment='left', verticalalignment='bottom', transform=ax1.transAxes)
			...
			ax3.text(0.02, 0.14, '%s' %nameDimO, fontsize=12, horizontalalignment='left', verticalalignment='bottom', transform=ax1.transAxes)
			ax3.text(0.02, 0.20, '%s' %nameDimM, fontsize=12, horizontalalignment='left', verticalalignment='bottom', transform=ax1.transAxes)
			...
			if given_filename is not None:
				filename = '%s.pdfunc-summary.plot%s.%s%s' %(given_filename, plotnr, dimspeci, pdfnames[1:])
			else:
				filename = '%s.pdfunc-summary.plot%s.%s%s' %(tablename, plotnr, dimspeci, pdfnames[1:])
		...
		ax1.text(0.02, 0.12, '%s' %nameDimO, fontsize=12, horizontalalignment='left', verticalalignment='bottom', transform=ax1.transAxes) 
		ax1.text(0.02, 0.18, '%s' %nameDimM, fontsize=12, horizontalalignment='left', verticalalignment='bottom', transform=ax1.transAxes) 
		...
		if given_filename is not None:
			filename = '%s.pdfunc-%s.plot%s.%s%s' %(given_filename, pdfsets[0], plotnr, dimspeci, ordernames[1:])
		else:
			filename = '%s.pdfunc-%s.plot%s.%s%s' %(tablename, pdfsets[0], plotnr, dimspeci, ordernames[1:])
...
def main():
	...
	ndim, binningdim, xlabel, all_bin_bounds, x_axis_list, xmin_list, xmax_list, x_errors, nplot, dimlabels, tmp_len_list, bbDimO, bbDimM = dimensionhandler(fnlo, verb)
	...
	for n in range(0, nplot):
		up_index = tmp_len_list[n] 	#tmp_len_list contains the last index (full arrays) for each bin of outer(!) dimension 
		...
		if (ndim>=2):
			...
			if (ndim==3):
				...
				print 'bin bounds DimM: low=%s, up=%s. '%(bbDimM_low, bbDimM_up)
		...
		if (ndim>1):
			if (ndim==2):
				nameDimO = fnlo.GetDimLabel(0)+': %s to %s'%(bbDimO_low, bbDimO_up) #name and range of outermost binning dimension
				nameDimM = '' #no middle dimension
				nameDimI = fnlo.GetDimLabel(1) #name of innermost dimension (== xlabel)

			elif (ndim==3):
				nameDimO = fnlo.GetDimLabel(0)+': %s to %s'%(bbDimO_low, bbDimO_up) #name of outermost binning dimension
				nameDimM = fnlo.GetDimLabel(1)+': %s to %s'%(bbDimM_low, bbDimM_up) #name of middle binning dimension
				nameDimI = fnlo.GetDimLabel(2) #name of innermost dimension (== xlabel)
		else:
			nameDimO = ''
			nameDimM = ''

...
double FastNLOReader::SetScaleVariation(int scalevar , bool ReFillCache ){ 
  ...
  fScalevar	= scalevar;
  fScaleFacMuR	= BlockB_NLO->ScaleFac[0][fScalevar];
  fScaleFacMuF	= BlockB_NLO->ScaleFac[0][fScalevar];
...
void FastNLOReader::PrintFastNLOTableConstants(const int iprint){
  ...
  if ( iprint > 1 ) {
    if ( BlockB_LO      ) BlockB_LO	->Print(1,iprint);
    if ( BlockB_NLO     ) BlockB_NLO	->Print(2,iprint);
...
void FastNLOReader::FillAlphasCacheInBlockBv20( FastNLOBlockB* B ){
  ...
  int scaleVar		= B->Npow == ILOord ? 0 : fScalevar;
  double scalefac	= B->ScaleFac[0][scaleVar] == fScaleFacMuR ? 1. : fScaleFacMuR;

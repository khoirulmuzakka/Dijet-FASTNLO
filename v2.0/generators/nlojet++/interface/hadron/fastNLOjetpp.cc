...
void inputfunc(unsigned int &nj, unsigned int &nu, unsigned int &nd) {
  ...
  if (!FastNLOUserHHC::instance->ftable) {
    ...
    // yet been read.
    cout << " # INIT:  [inputfunc] ---------- inputfunc called ----------"
         << endl;
    cout << " # INIT:  [inputfunc] ---------- initializing ... ----------"
         << endl;
    ...
    cout << " # INIT:  [inputfunc] ---------- LeadingOrder = " << ILOord
         << " ----------" << endl;
  }
...
void psinput(nlo::phasespace_hhc *ps, double &s) {
  //   cout << " # INIT:  [psinput] ---------- psinput called ----------" <<
  //   endl;
  ...
  say::debug["psinput"] << "cms energy read from steering: sqrt(s) = "
                        << FastNLOUserHHC::instance->ftable->GetEcms() << endl;
...
void FastNLOUserHHC::phys_output(const std::basic_string<char> &fname,
  ...
  //   cout << " # INIT:  [UserHHC::phys_output] ---------- UserHHC::phys_output
  //   called ----------" << endl;
  say::debug["UserHHC::phys_output"]
      << "---------- UserHHC::phys_output called ----------" << endl;
...
void FastNLOUserHHC::end_of_event() {
  say::debug["UserHHC::end_of_event"]
      << "---------- UserHHC::end_of_event called ----------" << endl;
  // --- count events
  nevents += 1;
  // --- store table

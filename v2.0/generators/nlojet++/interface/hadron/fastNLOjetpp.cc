...
void inputfunc(unsigned int& nj, unsigned int& nu, unsigned int& nd) {
   ...
   if (!FastNLOUserHHC::instance->ftable) {
      ...
      FastNLOUserHHC::instance->ftable = new fastNLOCreate("InclusiveNJets.str", GenConsts(), ProcConsts_HHC());
      if ( FastNLOUserHHC::instance->ftable->TestParameterInSteering("LeadingOrder") ) {
         FastNLOUserHHC::instance->ftable->GetParameterFromSteering("LeadingOrder",ILOord);
      } else {
...
void psinput(nlo::phasespace_hhc *ps, double& s) {
   ...
   say::debug["psinput"] << "cms energy read from steering: sqrt(s) = " << FastNLOUserHHC::instance->ftable->GetEcms() << endl;
...
void FastNLOUserHHC::phys_output(const std::basic_string<char>& fname, unsigned long nsave, bool txt) {
   ...
   say::debug["UserHHC::phys_output"] << "Before: __save = " << nsave << ", nwrite = " << nwrite << endl;
   nwrite = nsave;

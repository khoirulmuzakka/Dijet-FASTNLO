...
fastjet_jets::fastjet_jets(const JetAlgorithm jetalgo, const double jetsize, const double overlapthreshold)
   : jet_def(NULL), plugin(NULL) {
   setup(jetalgo, jetsize, overlapthreshold);
}

fastjet_jets::~fastjet_jets() {
   reset();
}
...
void fastjet_jets::setup(const JetAlgorithm jetalgo, const double jetsize, const double overlapthreshold) {
   reset();
...
fastjet_jets::operator()(const event_hhc& ev) {

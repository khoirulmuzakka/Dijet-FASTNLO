...
# Evaluate, keep only last 13 lines with numerical results
$cmd = "fnlo-tk-cppread ${tabfl} | tail -13 > cpptest.log";
print "Executing command: $cmd\n";
$ret = system("$cmd");
...
$cmd = "diff ../data/check/${tab}_cpp.log cpptest.log > cppdiff.log";
print "Executing command: $cmd\n";
$ret = system("$cmd");

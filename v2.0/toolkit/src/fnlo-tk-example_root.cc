...
int main(int argc, char** argv) {
   ...
   TH1D *histo1 = new TH1D("cross_section","cross_section",(int)cs.size()+1,fnlo.GetObsBinLoBound(0,0),fnlo.GetObsBinUpBound(cs.size()-1,0));
   for( int i = 0; i<(int)cs.size(); i++ ){
    histo1->SetBinContent(i+1,cs[i]);
   }

...
c     - new variables that did not exist in v14
     ...
     >     IAddMultFlag,IContrFlag1,IContrFlag2,IContrFlag3, NcontrDescr
     >     , NCodeDescr,IScaleDep, NPDF, NPDFPDG(2), NPDFDim, NFragFunc,
     >     NSubpr,IPDFCoeff, IPDFdef1, IPDFdef2, IPDFdef3,NScales,
     >     NScaleDim, nxall , nscaddr, nscvar
...
c     ------------------------------------------------------------------
      Write(2,'(I10)') Iseparator ! ------------------- block A1 ------
      Write(2,'(I5)') 20000     ! Itabversion
      Write(2,'(A)') 'FNX9999'  ! ScenName
      Write(2,'(I2)') Ncontrib  ! Ncontrib
      Write(2,'(I2)') 0         ! NMult
      Write(2,'(I2)') 0         ! NData
      Write(2,'(I2)') 0         ! Nuserstrings
c     Write(2,'(I2)') 1           ! Nuserstrings
c     Write(2,'(A)') 'converted_from_fastNLO_v14_table'   ! info      
      Write(2,'(I2)') 0         ! Nuserint
      Write(2,'(I2)') 0         ! Nuserfloat
      Write(2,'(I2)') 0         ! Imachine
c     ------------------------------------------------------------------
      Write(2,'(I10)') Iseparator ! ------------------- block A2 ------
      Write(2,'(I2)') IXSECTUNITS ! -> Ipublunits
      Write(2,'(I2)') 3         ! NScDescript 
      Do i=1,3
         Write(2,'(A)') NAMELABEL(i) ! ScDescript(i)
      Enddo
      Write(2,*) ECMS           ! Ecms
...
c     Do j=1,Ndim
      ...
      Write(*,*)
     >     '          double differential cross section, including'
...
c     ------------------------------------------------------------------
      Do n=nctrmin,nctrmax      ! loop over all contributions  B01...
         ...
         Write(2,'(I2)') IcontrFlag3 ! IcontrFlag3
         Write(2,'(I2)') NContrDescr ! NcontrDescr
         Do i=1,NContrDescr
            Write(2,'(A)') POWLABEL(n) ! CtrbDescript(i)
         Enddo
         Write(2,'(I2)') NCodeDescr ! NCodeDescr
ckr Replace with actual type of code description
ckr N = 1, 2: NLOJet++; N = 3: Threshold Corrections
         IF (N.LE.2) THEN
            CodeDescr(1) =
     >           "NLOJet++_2.0.1"
            CodeDescr(2) =
     ...
     >           "jet cross section in pp and ppbar"
            CodeDescr(3) =
     >           "N. Kidonakis, J.F. Owens, Phys. Rev. D63, "//
     >           "054019 (2001)."
         ELSE
...
c     Write(2,*) 1        ! Iscale(i) <- all scales = 1st Dimension
         ...
         Do i=1,Nscaledim
            ...
            Write(2,'(A)') SCALELABEL           
         Enddo
         
         If (IScaleDep.eq.0) then
            nscvar = 1
c     MW - iscdef not needed - just set MURSCALE = 1 for the LO
c     contribution
...
c     Endif
         ...
         Do i=1,Nscaledim
            if (nscvar.eq.1) then
c     MW               Write(2,*) MURSCALE(iscdef) ! Scalefac(i)(j)
               Write(2,*) 1d0   ! Scalefac(i)(j)
c     debug
c     debug               Write(*,*)"CONV20: IScaleDim,IScaleVar
c     ,ScaleFac: ",
c     debug     >              i,iscdef,MURSCALE(iscdef)
c     debug
            else
               Do j=1,nscvar
...
c     --- special cases: in pp in LO merge 2nd & 3rd subproc for pp
c     /ppbar ---
...
ckr 5000 FORMAT (I5)               ! for integers < 100k
ckr 5001 FORMAT (I10)              ! for Iseparator 
ckr 5002 FORMAT (F20.18)           ! for long positive real 
ckr 5003 FORMAT (D24.17)           ! for coefficients
ckr 5009 FORMAT (A)                ! for charactrs & zeros (printed as character)
...
*-----------------------------------------------------------------
*     M. Wobisch   read ASCII table of perturbative coefficients from
*     v1.4
*     
*     input: FILENAME  name of table
*     
...
*-----------------------------------------------------------------
     ...
     +        '  -  IOSTAT = ',IFILE
      ...
      Read(2,*) ITABVERSION
      Write(*,*) "#       tableformat is version",real(itabversion)
     >     /10000d0
      if (ITABVERSION.ne.14000 .and. ITABVERSION.ne.16000) then
         Write(*,*)
     >        '#     => this conversion works only for v1.4 and v1.6'
         stop
      endif
      Read(2,*) i
      if (i.ne.iseparator) goto 999

//
// fastNLO v2 creator code for fnl2412c:
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
   ...
   if (njet > 1) {
      ...
      double mjj = pj12.mag();
      if (mjj < 0.) {cout << "fastNLO: WARNING! Negative mass encountered: " << mjj << endl;}
      ...
      if ( ptj1min <= pT1 && mjjmin <= mjj ) {

         // --- set the renormalization and factorization scale to average dijet pT
         double mu = pT12;

         // --- identify bin number (dim1,dim2) here (mjj,|y_max|)
         int obsbin = -1;
...
void UserHHC::inittable(){
   ...
   A2->ScDescript.push_back("d2sigma-dijet_dMjjdy_max_[pb_GeV]");
   ...
   A2->ScDescript.push_back("CMS-PAPER-QCD-10-025, arXiv:1104.1693, Phys. Lett. B 700 (2011) 187.");
   ...
   const int ndim1bins[ndim2bins] = { 32, 31, 24, 20, 18 };
   ...
   const double dim0[33] = {
      156. ,  176.,  197.,  220.,  244.,  270.,  296.,  325.,  354.,  386.,
      419. ,  453.,  489.,  526.,  565.,  606.,  649.,  693.,  740.,  788.,
      838. ,  890.,  944., 1000., 1118., 1246., 1383., 1530., 1687., 1856.,
      2037., 2332., 2659.
   };
   const double dim1[32] = {
      197. ,  220.,  244.,  270.,  296.,  325.,  354.,  386.,  419.,  453.,
      489. ,  526.,  565.,  606.,  649.,  693.,  740.,  788.,  838.,  890.,
      944. , 1000., 1118., 1246., 1383., 1530., 1687., 1856., 2037., 2332.,
      2659., 3019.
   };
   const double dim2[25] = {
      386. ,  419.,  453.,  489.,  526.,  565.,  606.,  649.,  693.,  740.,
      788. ,  838.,  890.,  944., 1000., 1118., 1246., 1383., 1530., 1687.,
      1856., 2037., 2332., 2659., 3019.
   };
   const double dim3[21] = {
      565. ,  606.,  649.,  693.,  740.,  788.,  838.,  890.,  944., 1000.,
      1118., 1246., 1383., 1530., 1687., 1856., 2037., 2332., 2659., 3019.,
      3854.
   };
   const double dim4[19] = {
      649. ,  693.,  740.,  788.,  838.,  890.,  944., 1000., 1118., 1246.,
      1383., 1530., 1687., 1856., 2037., 2332., 2659., 3019., 3854.
   };
   ...
   //
   //     fnl2412: Divide by bin width in Mjj (1st dimension) and y_max (2nd dimension)
   //
   ...
   for (int i=0;i<ndim2bins;i++){
      for (int j=0;j<ndim1bins[i];j++){
         ...
         binsize = binsize
            ...
            * 2. * (dim2bins[i+1]-dim2bins[i]); // ... times dy_max

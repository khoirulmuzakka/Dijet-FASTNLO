...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
  ...
  if (nj > 0) { // else nothing to fill
    ...
    // fnl2822den: Maximal 3 partons = jets
    int njmax = 3;
	...
	// fnl2822den: jet pT, leading jet number
...
void UserHHC::inittable(){
  ...
  table->GetBlockA1()->SetScenName("fnl2822den");  // - fastNLO user: set scenario name
  ...
  A2->ScDescript.push_back("2->2_&_2->3_Processes");
  ...
  double rapbins[nrapbins+1] = {0.5, 1.5, 2.5, 3.5};
  
  const int nptbins[nrapbins] = {49, 49, 49};
  ...
  // fnl2822den: range in |y| is 6. here
  double binsize = 6.;
  ...
  for (int i=0;i<nrapbins;i++){
    for (int j=0;j<nptbins[i];j++){
      ...
      //printf(" %d %d  |  %f %f\n",i,j,bound[0],bound[1]);
      binsize = binsize // fnl2822den: Start with preset value from |y| range 

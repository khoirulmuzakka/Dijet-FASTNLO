//
// fastNLO v2 author code for fnl2732denb:
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
  ...
  // fnl2732: Required for 3-jet ratio
...
void UserHHC::inittable(){
  ...
  A2->DimLabel.push_back("<pT_1,2>_[GeV]");  // --- fastNLO user: label of 1st dimension
  A2->IDiffBin.push_back(2);
  A2->DimLabel.push_back("N_Jet_Min");   // --- fastNLO user: label of 2nd dimension
  A2->IDiffBin.push_back(2);

  vector <double> bound;
  bound.resize(2);

  // --- fastNLO user: bin definitions - here in <pT_1,2> and minimal jet multiplicity
  ...
  // fnl2732: divide by bin width in pT but NOT in |y|
  ...
  for (int i=0;i<ndim2bins;i++){
	...
	* (dim1bins[i][j+1]-dim1bins[i][j]); // ... times dpT
      //	* (dim2bins[i+1]-dim2bins[i]); ... times 2nd dimension ?

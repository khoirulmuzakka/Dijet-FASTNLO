...
void UserHHC::inittable(){
  ...
  // fnl2422: divide by bin width in m12 and y*
  ...
  for (int i=0;i<ndim2bins;i++){
    for (int j=0;j<ndim1bins[i];j++){
      ...
      binsize = binsize
	* (dim1bins[i][j+1]-dim1bins[i][j]) // ... times dm12
	* (dim2bins[i+1]-dim2bins[i]); // ... times dy*

//
// fastNLO author code for fnl1308:
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
  ...
  double pTmin = 53.0, ymin = 0.00, ymax = 0.55;
...
void UserHHC::inittable(){
  ...
  table->GetBlockA1()->SetScenName("fnl1308");
  ...
  A2->ScDescript.push_back("CMS-Collaboration");
  A2->ScDescript.push_back("");
  A2->NScDescript = A2->ScDescript.size();
  A2->Ecms = sqrt(s);
  A2->ILOord = 2;
  A2->NDim = 2;
  A2->DimLabel.push_back("p_T");
  ...
  double rapbins[nrapbins+1] = {0.00,0.55};

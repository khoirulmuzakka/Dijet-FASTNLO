*******************************************************************
*******************************************************************
*                                    T. Kluge, M. Wobisch 06/05/2006      
*                            
* fastNLO user code  
*         -> special version for scenario fnt2001 which is
*            the ratio of two cross sections (fnt2001a, fnt2001b) 
*
...
*******************************************************************
      SUBROUTINE FT2001CC(FILENAME1, FILENAME2, 
     +     XMUR, XMUF, IPRINTFLAG, XSECT)
*-----------------------------------------------------------------
* fastNLO user code - main routine 
*
* input:
*   FILENAME1    name of 1st input table (DeltaPhi cross section)
*   FILENAME2    name of 2nd input table (normalization)
...
*-----------------------------------------------------------------
      IMPLICIT NONE
      INTEGER j, IPRINTFLAG, IBIN(4), ino
      CHARACTER*(*) FILENAME1, FILENAME2
      DOUBLE PRECISION xmur, xmuf, array1(94,3), array2(4,3),xsect(94,3)
      DATA IBIN/18,41,68,94/

      call FT2001aCC(FILENAME1, xmur,xmuf, 0, array1)
      call FT2001bCC(FILENAME2, xmur,xmuf, 0, array2)

      do j=1,94
         if (j.le.ibin(1)) then
            ino=1
         elseif (j.le.ibin(2)) then
            ino=2
         elseif (j.le.ibin(3)) then
            ino=3
         elseif (j.le.ibin(4)) then
            ino=4
         endif
         xsect(j,1) = array1(j,1)/array2(ino,1)
         xsect(j,2) = (array1(j,1)+array1(j,2))/
     +        (array2(ino,1)+array2(ino,2)) - xsect(j,1)
         xsect(j,3) = 0d0
      enddo

      if (iprintflag.eq.1) then
         write(*,*) '--  fastNLO - results for ',
     +        '1/sigma dsigma/dDelta Phi'
         write(*,*) '--------  muf/mu0= ',xmuf,'    mur/mu0= ',xmur
         write(*,*) '--              from 0.0 to 0.5 in |y|'
         write(*,*) '--    results:         LO          NLOcorr ',
     +        '     total'
         do j=1,94
            if (j.eq.1)  write(*,*) ' --    75 < pTmax < 100 GeV'
            if (j.eq.19) write(*,*) ' --   100 < pTmax < 130 GeV'
            if (j.eq.42) write(*,*) ' --   130 < pTmax < 180 GeV'
            if (j.eq.69) write(*,*) ' --   pTmax > 180 GeV'
            write(*,*) 'DeltaPhi bin No.',j,' ',xsect(j,1),xsect(j,2),
     +           (xsect(j,1)+xsect(j,2))
         enddo
         write(*,*)
      endif
...
*******************************************************************
      SUBROUTINE FT2001ACC(FILENAME, XMUR, XMUF, IPRINTFLAG, XSECT)
*-----------------------------------------------------------------
* fastNLO user code - main routine 
*
* input:
...
c - now get the PDFs ...
      call FT2001AGP(mufscale(ixmuf))
c - ... and multiply with perturbative coefficients 
      call FT2001AMT(xmur,ixmuf)
...
c - print results - if requested
      if (IPRINTFLAG.eq.1) call FT2001APR(xmur,IXMUF)
...
*******************************************************************
      SUBROUTINE FT2001AMT(xmur,ixmuf)
*-----------------------------------------------------------------
* MW 08/26/2005
*
...
*******************************************************************
      SUBROUTINE FT2001AGP(muffactor)
*-----------------------------------------------------------------
* MW 08/26/2005
*
...
c - now fill main PDF array - compute different lin. comb for diff sub-proc
               ...
               do k=1,NXTOT
                  ...
                  do l=1,nx2limit
                     nx=nx+1
                     call ft2001apl(ireaction,k,l,XPDF,H)
                     do m=1,Nsubproc
                        pdf(nbin,nx,m,p) = H(m)
                     enddo
                  enddo
...
*******************************************************************
      SUBROUTINE FT2001APL(ireact,i,j,XPDF,H)
* ---------------------------------------------------------------
* MW 03/27/06
...
*******************************************************************
      ...
      SUBROUTINE FT2001APR(XMUR,IMUFFLAG)
*-----------------------------------------------------------------
...
*******************************************************************
      SUBROUTINE FT2001BCC(FILENAME, XMUR, XMUF, IPRINTFLAG, XSECT)
*-----------------------------------------------------------------
* fastNLO user code - main routine 
*
* input:
...
c - now get the PDFs ...
      call FT2001BGP(mufscale(ixmuf))
c - ... and multiply with perturbative coefficients 
      call FT2001BMT(xmur,ixmuf)
...
c - print results - if requested
      if (IPRINTFLAG.eq.1) call FT2001BPR(xmur,IXMUF)
...
*******************************************************************
      SUBROUTINE FT2001BMT(xmur,ixmuf)
*-----------------------------------------------------------------
* MW 08/26/2005
*
* multiply the PDFs and the coefficients for a single scale setting
...
*******************************************************************
      SUBROUTINE FT2001BGP(muffactor)
*-----------------------------------------------------------------
* MW 08/26/2005
*
* read the PDFs in all (rap,pT) bins - at all xmax, xmin bins
...
c - now fill main PDF array - compute different lin. comb for diff sub-proc
               ...
               do k=1,NXTOT
                  ...
                  do l=1,nx2limit
                     nx=nx+1
                     call ft2001bpl(ireaction,k,l,XPDF,H)
                     do m=1,Nsubproc
                        pdf(nbin,nx,m,p) = H(m)
                     enddo
                  enddo
...
*******************************************************************
      SUBROUTINE FT2001BPL(ireact,i,j,XPDF,H)
* ---------------------------------------------------------------
* MW 03/27/06
* compute PDF linear combinations - for different reactions
*
...
*******************************************************************
      ...
      SUBROUTINE FT2001BPR(XMUR,IMUFFLAG)
*-----------------------------------------------------------------
* m. Wobisch  - print fastNLO cross section results
*
* input:

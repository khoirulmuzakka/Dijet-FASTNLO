//
// fastNLO author code for fnr0001 - RHIC sqrt(s)=200GeV - 3 algos
//
// last modification 2005/11/04 - MW
...
void UserHHC::initfunc(unsigned int)
{
   ...
   for( int j = 0; j < nrap; j++) {
      ...
      for( int k = 0; k < npt[j]; k++) {
         ...
         xlimit[j][k] = xmin;
	 if (j==0) {
	   xlimit[j][k] = xlimit[j][k]*0.85; // safety factor for ET-scheme
	 }
	 if (j==1) {
	   xlimit[j][k] = xlimit[j][k]*0.76; // safety factor for ET-scheme
	 }
	 if (j==2) {
	   xlimit[j][k] = xlimit[j][k]*0.58; // safety factor for ET-scheme
	 }
		 ...
		 ptbin=j;
            ...
            if ( ptbin>=0 ) {
               ...
               if (xmin<xlimit[rapbin][ptbin]){
                  printf("Warning: xmin (%f) < xlimit (%f) at pt=%f GeV y=%f algo=%2d \n ",
                         xmin,xlimit[rapbin][ptbin],pt,rap,rapbin);
                  exit(1);
               }
               ...
               if (xmin<xsmallest[rapbin][ptbin]*0.98){  //  0.98 reduc output!
                 ...
                 if((itype==amplitude_hhc::lo && nevents> 700000000)|| //output
                  (itype==amplitude_hhc::nlo && nevents> 70000000)){  //after3h
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   // ireaction
   int ireaction = 2;     // 1: DIS     2: pp     3: ppbar
   WRITE(ireaction);

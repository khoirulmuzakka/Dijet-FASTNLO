...
void UserHHC::initfunc(unsigned int)
{
   ...
   nrap   = 5*2;                    // no of bins in rapidity (5 for D0) plus the same for reference
		  ...
		  //  in reference jobs:  comment the following line
		    ...
		    (1-deltamax)*(1-deltamin)*wt;
		       ...
		       (1-deltamax)*deltamin*wt;
                  ...
                  wt = amp(mur2,muf2);
                  // ** ----------------------------------------------------------
                  // **   now fill half-table with pdfs and alphas
                  weights[rapbin+nrap/2][ptbin][nxmax][nxmin][scalevar] += 
		    (1-deltamax)*(1-deltamin)*wt;

                  // -- avoid writing behind  nxmax=(nxtot-1)
                  if (nxmax<(nxtot-1)) {
                     weights[rapbin+nrap/2][ptbin][nxmax+1][nxmin][scalevar] += 
		       deltamax * (1-deltamin)*wt;
                     weights[rapbin+nrap/2][ptbin][nxmax+1][nxmin+1][scalevar] += 
		       deltamax * deltamin*wt;
                  }

                  // -- avoid writing behind  nxmin=nxmax
                  if (nxmin<nxmax) {             // should work with half-table
                     weights[rapbin+nrap/2][ptbin][nxmax][nxmin+1][scalevar] += 
		       (1-deltamax)*deltamin*wt;
                  }

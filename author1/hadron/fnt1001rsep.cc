//
// fastNLO author code for fnt1001 for cone algo with Rsep=1.3
//                     Tevatron Run I jets D0 and CDF
//
// last modification 
// 2005/12/07 MW - include new xlimit calculation
...
void UserHHC::initfunc(unsigned int)
{
   ...
   for( int j = 0; j < nrap; j++) {
      ...
      for( int k = 0; k < npt[j]; k++) {
         ...
         xlimit[j][k] = xmin;
         xlimit[j][k] = xlimit[j][k]*0.88; // safety factor for ET-scheme
   ...
   printf ("#rap #pt xlimit_cor pt_high rap_high \n");
   for( int j = 0; j < nrap; j++) {
      for( int k = 0; k < npt[j]; k++) {
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   //JetResol2
   double JetResol2 = 1.3;  // Rsep value
   WRITE(JetResol2);

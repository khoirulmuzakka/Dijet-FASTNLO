...
c ---------------------------------------------------------------------
     ...
     +     rs,y,ymin,ymax,ytest,yup,xt,pt,pt2,ptmin,ptmax
      ...
      Double Precision binwidth, norm, xs(9),xs1(9),xs2(9)
      CHARACTER*100 INTABLE, OUTTABLE, Histofile
      INTEGER IOPTIMIZE
...
c      ns4pt = 40
      ...
      if (ioptimize.eq.1) then
         ...
         write(*,*) '   - and the O(alphas^4) coefficients are stored'
         write(*,*) '     in the fastNLO table'
         write(*,*) ' '         
      endif

      IF ( IARGC().EQ.0)  THEN
         write(*,*) ' '
         write(*,*) ' the code requires 3 arguments:'
         write(*,*) '      [input-fastNLO-table]   (required)'
         write(*,*) '      [output-text-table]     (optional)'
         write(*,*) '      [output-histogram-file] (optional)'
         write(*,*) ' '
         stop
      ENDIF
      IF ( IARGC().LT.1)  INTABLE = 'table.tab'
      IF ( IARGC().LT.2)  OUTTABLE= 'thrcor-table.txt'
      IF ( IARGC().LT.3)  HISTOFILE= 'threshcor.hbk'
      IF ( IARGC().GT.0)  CALL GETARG(1,INTABLE)
      IF ( IARGC().GT.1)  CALL GETARG(2,OUTTABLE)
      IF ( IARGC().GT.2)  CALL GETARG(3,HISTOFILE)

      write(*,*) ' getting phase space from fastNLO table ',intable
      write(*,*) ' store O(alphas^4) coefficients in table ',outtable
...
c --- special treatment for CDF bin in Scenario "fnt1001"
            if (irap.eq.6 .and. ymax.lt.ymin) then
               ymin = 0.1
            endif
            ...
            do ipt=1,npt(irap)  ! loop over pT bins
               ...
               binwidth = (ptmax-ptmin)*2d0*(ymax-ymin)
...
c                  if (norm<1.d0) write(*,*) ' reduced accessible y range:'
c     +                 ,irap,ipt,i,norm
...
c - not possible in v1 of fastNLO
c                        mur = murval(irap,ipt) * murscale(iscale)
c - in v1 this is required (no aposteriori scale variation for thrsh-cor)
                        mur = muf ! fn: unless we vary mur later independently
                        mur2=mur*mur
                        muf2=muf*muf
...
c -                     contribution for fastNLO table
                           xs(ipdf) = cf(ipdf)*xabin/dble(npoint)
     +                          * (SIG0(ipdf)*
     +                          (C20(1,ipdf)*ALS4M
     +                          +C20(2,ipdf)*ALS4M**2/2
     +                          +C20(3,ipdf)*ALS4M**3/3.
     ...
     +                          * FAC2 * 4d0 * binwidth * Nevents
c     +                          * AS*AS*AS*AS/4d0/4d0/PI/PI/PI/PI
c     +                          * HAB0(ipdf)

                        enddo
c - fill this contribution into the fastNLO table
                        call FILLTABLE(xa,xb0,xs,nbin,irap,ipt,iscale)
...
c - proportional to HAB0
                              xs1(m) =-cf(m)*s4bin*xabin/dble(npoint) / S4
     +                             *(SIG0(m)
...
c - proportional to HAB1
                              xs2(m) = cf(m)*s4bin*xabin/dble(npoint) / S4
     +                             *(SIG1(m)
     +                             *(C2(1,m) + C2(2,m)*ALS4
...
c - fill these contributions into the fastNLO table
                        call FILLTABLE(xa,xb0,xs1,nbin,irap,ipt,iscale)
                        call FILLTABLE(xa,xb,xs2,nbin,irap,ipt,iscale)
...
C ----------------------------------------------------------
      SUBROUTINE FILLTABLE(xa,xb,coef,nbin,nrapbin,nptbin,nscale)
c -------------------------------------------
c MW 
...
c -------------------------------------------
      implicit none
      integer nbin,nrapbin,nptbin,nscale,nf
...
c - first table element
         table(nbin,nmax,nmin,nscale,isub) = 
     +           table(nbin,nmax,nmin,nscale,isub) 
     +           + cf(isub) * (1d0-deltamax) * (1d0-deltamin)

c --- avoid writing behind nmax=nxtot
         if (nmax.lt.nxtot) then
            table(nbin,nmax+1,nmin,nscale,isub) = 
     +           table(nbin,nmax+1,nmin,nscale,isub) 
     +           + cf(isub) * deltamax * (1d0-deltamin)
            table(nbin,nmax+1,nmin+1,nscale,isub) = 
     +           table(nbin,nmax+1,nmin+1,nscale,isub) 
     +           + cf(isub) * deltamax * deltamin
         endif

c --- avoid writing behind nmin=nmax
         if (nmin.lt.nmax) then
            table(nbin,nmax,nmin+1,nscale,isub) = 
     +           table(nbin,nmax,nmin+1,nscale,isub) 
     +           + cf(isub) * (1d0-deltamax) * deltamin
         endif

c --- compensate for missing contribution: nmin>nmax (use symmetry)
c -   ---> project  (nmax,nmin+1) back to (nmax+1,nmin)
         if (nmin.eq.nmax .and. nmin.lt.nxtot) then
c  -   don't need to swap here: 2,3 are identical for threshold corr.
         table(nbin,nmax+1,nmin,nscale,isub) = 
     +           table(nbin,nmax+1,nmin,nscale,isub) 
     +           + cf(isub) * (1d0-deltamax) * deltamin
         endif         
...
c -------------------------------------------
      ...
      CHARACTER*(*) filename
      CHARACTER*(33) label
      integer i,j,k,l,m, isub, nbin
      include 'fnthresh.inc'

      nbin=0
      label='NNLO-NLL-(threshold-corrections)'

...
*---  is (0,0) and is not to be used!
      INTEGER MXSCALECOMB
      PARAMETER (MXSCALECOMB=2*NSCALEMAX)
...
*---  Look for pointers matching requested scale settings
      DO ISCL=1,NSCLS
         ...
         XMUF = XMUFS(ISCL)
         DO J=1,NSCALEVAR
            IF (ABS(MUFSCALE(J)-XMUF).LT.TINY(1.D0)) THEN
               ISCLPT(ISCL) = J
               EXIT
            ENDIF
         ENDDO
         IF (ISCLPT(ISCL).LE.0) THEN
...
*---  Book histograms
     ...
     >     LRAT.OR.LNRM,LSCL)
      WRITE(*,'(A,I4,A)')" ALLUNC: The observable has",NBINTOT," bins."
      WRITE(*,'(A,I2,A)')" ALLUNC: The  LO is subdivided into ",
     >     NSUBPROC," subprocesses and"

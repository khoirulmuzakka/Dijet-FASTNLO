//
// fastNLO author code for fnt200a
//
//    Run II preliminary D0 incl jet results
// 
// last modification
...
void UserHHC::initfunc(unsigned int)
{
   ...
   //Define binning in pT   - start at pT=10 - 613 bins
   npt[0]=144;   // up to 730
   npt[1]=130;   //       660
   npt[2]=108;   //          550 (not used in analysis)
   npt[3]=86;   //          440 (not used in analysis)
   npt[4]=66;   //       340
   npt[5]=46;   //       240
   npt[6]=33;   //       175
   ...
   for( int j = 0; j < nrap; j++) {
	 ...
	 xlimit[j][k] = xlimit[j][k]*0.98; // small safety factor -> E-scheme
         
         hxlim[j][k]= -sqrt(-log10(xlimit[j][k]));
         xsmallest[j][k]=0.999;

         // - Setup the murval and mufval arrays - take value at 48% of bin
         murval[j][k]= (0.52*pt + 0.48*pthigh[j][k+1]); 
         mufval[j][k]= (0.52*pt + 0.48*pthigh[j][k+1]); 
      }
   }
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
   ...
   // code to copy parton in 2-parton config.
   /*
   ...
   case 2:
     _M_pj.push_back(_M_p[1]);
     _M_pj.push_back(_M_p[2]);
     break;
   case3:
     // Hier primitiv pxcone
     break;
   default:
     //Und hier kommt der vorhandene Code hin
   }
   */
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
  ...
  //iproc
   ...
   //JetResol2
   double JetResol2 = 0.75; // midpoint cone: f_overlap (not effective up to NLO)
   WRITE(JetResol2);

...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
  ...
  double ptxmin = 125.;
  // minimal pT fraction of 3rd jet in order to be counted
  double pt3frac = 0.25;
  ...
  if (nj > 0) {
	...
	if ( pt4 > pt1 ) {
      ...
      //     int njc = 2;
      //
      // Translation of requirements according to Panos & Costas into (pT1+pT2)/2
      ...
      double yjet[4]  = {abs(pj[ij1].rapidity()),
			 ...
			 abs(pj[ij3].rapidity()),
			 abs(pj[ij4].rapidity())};
      double pT12ave = (pt1+pt2)/2.;
      for (int i=1; i<=njet; i++) {
	if ( yjet[i-1] < yjcmax ) {
	  if ( i < 3 ) {
	    njc++;
	    nlj++;
	  } else if (ptjet[i-1]/pT12ave > pt3frac ) {
	    njc++;
	  }  
	}
      }
      ...
      //     cout << "njc, pT12ave: " << njc << ", " << pT12ave << endl;
      //DEBUGEND

      if ( nlj == 2 && njc > 2 && pT12ave > ptxmin ) {
	//DEBUG
	//       cout << "Event accepted: " << endl;
	//       cout << "njet, njc, pt1, pt2, yjjmax, pT12ave: " << njet << ", " << njc << ", " << pt1 << ", " << pt2 << ", " << yjjmax << ", " << pT12ave << endl;
	//       cout << "==================== End of event ====================" << endl; 
	//DEBUGEND
	
	// --- determine y(=mult) and pT(=pT12ave) bin (njc = 2+) 
	...
	//DEBUG
	//	 if ( ptbin>0 ) {
	//	   cout << endl << "pT12avelow = " << pthigh[rapbin][ptbin] << ", pT12ave = " << pT12ave << ", pT12avehig = " << pthigh[rapbin][ptbin+1] << endl;
	//	 } else {
	//	   cout << endl << "pT12ave = " << pT12ave << endl;
	//	 }
	//DEBUGEND
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
  ...
  table << "dsigma-jet3+_dpT12ave_(fb_GeV)" << endl;
  ...
  // absolute order
  int npow = nord+2; // -> here "+2" for three-jet xsect (pT12ave)
  WRITE(npow);

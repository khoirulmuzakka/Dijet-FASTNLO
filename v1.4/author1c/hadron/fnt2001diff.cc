//
// fastNLO author code for fnt2001 - differential
//
//    Run II first D0 QCD jet publication - Dijet Delta Phi
//          hep-ex0409040
// 
// last modification
...
void UserHHC::initfunc(unsigned int)
{
   ...
   //Set up binning  
   nrap   = 4;                 // No. of bins in rapidity -> here: pT
   ...
   raphigh[0]=75.0;        // here: these are pT bins!!!
   raphigh[1]=100.0;        // 
   raphigh[2]=130.0;        // 
   raphigh[3]=180.0;        // 
   raphigh[4]=980.0;        // 
   ...
   for(int i=0;i<nrap;i++){
     for(int j=0;j<npt[i]+1;j++){
       pthigh[i][j] = pthigh[i][j] * pi/128.0;
     }
   }

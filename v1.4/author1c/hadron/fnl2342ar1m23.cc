//
// fastNLO author code for fnl2342ar1m23:
//     CMS LHC Leading Jets Scenario, E_cms = 7 TeV
...
void UserHHC::initfunc(unsigned int)
{
   ...
   // Here: Leading jet binning no. 1, 2, 3 
   // # of bins
   nrap = 3;
   double rapb[4] = { 0.5, 1.5, 2.5, 3.5 };
   ...
   for( int j = 0; j < nrap; j++) {
	 ...
	 if ( (ymin > ymax) && (iref == 0 || (iref == 1 && j < nrap/2 ) ) ) {
	   ...
	   cout << "The rapidity bin runs from " << yjmin <<
	     " to " << yjmax << endl;
   ...
   cout << "    Scenario fnl2342ar1m23:" << endl;
   cout << "      CMS LHC Leading Jets Scenario, E_cms = 7 TeV," << endl;
...
// Initialize pointers to the jets, check minimal jet pT and maximal |y,eta|
   ...
   if (njet >= 2) {
     ...
     // Reuse ijet array as pointer this time
     ijet[1] = ij1;
     ijet[2] = ij2;
     ijet[3] = ij3;
     ijet[4] = ij4;
     // Only central rapidity bin
     double ybmax = 0.5;
     for (int i = 1; i <= njet; i++) {
       
       // Get jet properties
       double pt  = pj[ijet[i]].perp(); 
       double rap = abs(pj[ijet[i]].rapidity());
       if (pt > ptlow && rap < ybmax) {
       
	 // Determine y (= "leading" bin = i) and pt bin
	 // Do normalize to binwidths! |y| bin width = 1
	 double binwidth = 1.0;
	 int rapbin = i;

...
class UserHHC : public user_hhc
{
 ...
 private:
  ...
  vector< vector<double> > bicef;
   ...
   pdf_cteq6 pdf;  //   pdf
   fj_kt_10 jetclus;   // jet algorithm
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   table << "ATLAS-LHC-Test-Scenario" << endl;
  ...
  //iproc
   ...
   //ialgo
   int ialgo = 1; // midpoint cone algo
   WRITE(ialgo);

   //JetResol1
   double JetResol1 = 1.0;  // Jet size R
   WRITE(JetResol1);

   //JetResol2
   double JetResol2 = 0.0; // Overlap threshold
   WRITE(JetResol2);

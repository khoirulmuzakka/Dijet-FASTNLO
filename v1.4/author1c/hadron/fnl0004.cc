//
// fastNLO author code for fnl0004 - ATLAS test scenario
//       for kT algo with D=0.7   in E-scheme
// 
// last modification
...
void UserHHC::initfunc(unsigned int)
{
   ...
   //----- array for pt boundaries - include two more bins at edge
   for (int j=0; j<nrap; j++){
     ...
     pthigh[j][4] =  160.0;
     pthigh[j][5] =  200.0;
     for (int i=6; i<min(12,npt[j]); i++){    // 50 GeV bins -> 500GeV
       pthigh[j][i] = pthigh[j][i-1] + 50.0;
     }  
     for (int i=12; i<min(17,npt[j]); i++){  // 100 GeV bins -> 1000GeV
       pthigh[j][i] = pthigh[j][i-1] + 100.0;
     }  
     for (int i=17; i<min(27,(npt[j])); i++){  // 200 GeV bins -> 3000GeV
       pthigh[j][i] = pthigh[j][i-1] + 200.0;
     }  
     for (int i=27; i<min(99,(npt[j])); i++){  // 500 GeV bins -> ...
       pthigh[j][i] = pthigh[j][i-1] + 500.0;
     }  
   }

   pthigh[0][npt[0]] = 4500.0;   
   pthigh[1][npt[1]] = 3500.0;   
   pthigh[2][npt[2]] = 2200.0;   
   pthigh[3][npt[3]] = 1100.0;   
   pthigh[4][npt[4]] =  600.0;   
   ...
   ptsmallest.resize (nrap);     // test: find smallest pT values in bins
   ptlargest.resize (nrap);     // test: find largest pT values in bins
   pthi.resize (nrap);     //
   ptlo.resize (nrap);     //
   murval.resize (nrap);
   mufval.resize (nrap);
   ...
   for( int j = 0; j < nrap; j++) {
	 ...
	 for (int nr = 0; nr <= 500; nr++) {
	   double ytest = ymin + double(nr)*(ymax-ymin)/500.0;
	 ...
	 if (xlimit[j][k] > 0.99999) xlimit[j][k]=0.95;  // for empty bins
	 if (xlimit[j][k] < 0.0) xlimit[j][k]=0.95;  // for empty bins

         // define reasonable upper and lower pT boundaries for each bin
	 //      -> not really used ...
         pthi[j][k]  = pthigh[j][k+1];
         ptlo[j][k]  = pthigh[j][k];
         ...
         murval[j][k][0] = ptlo[j][k] +dpt*0.1;
         murval[j][k][1] = pthi[j][k] -dpt*0.2;
   ...
   cout<<"         *** scenario fnl0004 ***"<<endl;
   cout<<"            ATLAS test scenario"<<endl;
   cout<<"        >>> true running scale mu=pT"<<endl;
   cout<<" "<<endl;
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
   ...
   for( int i = 1; i <= nj; i++) {
	 ...
	 if (iref==1) nloop = nrap/2; // -> in reference mode
	    ...
	    // test only: determine absolute cross section in each bin
	    //binwidth = 1.0;
	    //binwidth = 0.0001;   // No. of events for L=10fb-1
		 ...
		 for( int i2 = 0; i2 < 4; i2++) {
		  ...
		  // -> later: replace "deltascale" with Bernstein-Polynomial 
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   table << "not-published" << endl;
   table << "ATLAS-test-scenario" << endl;
   table << "-" << endl;
   table << "-" << endl;

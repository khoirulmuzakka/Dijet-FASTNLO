//
// fastNLO author code for fnl2722b:
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
  ...
  // lowest pT for central jets
  double ptclow = pthigh[0][0];
  // lowest pT for forward jets
  double ptflow = pthigh[nrap-1][0];
  // lowest pT for all
  double ptlow = min(ptclow,ptflow);
  ...
  //   cout << "ptclow, ptflow, ptlow, pthig = " << ptclow << ", " << ptflow << ", " << ptlow << ", " << pthig << endl;
  ...
  if (nj > 0) {
	...
	if ( pt4 > pt1 ) {
      ...
      // For the two leading jets set central one to come first
	...
	pt2 = pttmp;
      ...
      //DEBUG
      //       cout << "pT,|y| ordered leading jets before cuts: " << endl;
      ...
      if ( ptmax < pthig && ptclow < pt1 && ptflow < pt2 && y1 < yjcmax && yjfmin < y2 && y2 < yjmax ) {

	// Analyze leading jets in jet loop
	for (int i = 1; i <= nj; i++) {
	  // Only the leading jets
	  if (i==ij1 || i==ij2) {
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
  ...
  // absolute order
  int npow = nord+1; // -> here "+1" for two-jet xsect
  WRITE(npow);

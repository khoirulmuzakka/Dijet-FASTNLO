//
// fastNLO author code for fnl2518 (differential in dPhi)
//
//    LHC @ 7 TeV test scenario
...
void UserHHC::initfunc(unsigned int)
{
   ...
   //Set up binning  
   nrap   = 1;                 // No. of bins in rapidity -> here: pT
   ...
   raphigh[0]=120.0;        // here: these are pT bins!!!
   raphigh[1]=200.0;        // 
   ...
   // Cosmin: 300-500 :    45*p    55*p    66*p    70*p    72*p    74*p
   //    76*p    77*p    78*p    79*p    80*p    81*p    82*p    83*p
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   table << "Differential, second pT bin" << endl;

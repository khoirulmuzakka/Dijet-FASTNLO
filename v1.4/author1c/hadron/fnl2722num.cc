...
void UserHHC::initfunc(unsigned int)
{
   ...
   // Here: Two pseudo-bins in rapidity (no normalization to bin widths should be done!):
   //       1) 3+ jets in |y| < 2.5
   //       2) 2  jets in |y| < 2.5
   // # of bins
   nrap = 2;
...
// Initialize pointers to the jets, check minimal jet pT and maximal |y,eta|
   ...
   if (njet >= 2) {
     ...
     if (yjjmax < yjcmax && ht > htmin ) {
       ...
       // --- determine y(=multiplicity) and pT(=HT) bin (njc = 3+ in rapbin 0, njc = 2 in rapbin 1) 
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   table << "dsigma_3+_dHT_(pb_GeV)" << endl;

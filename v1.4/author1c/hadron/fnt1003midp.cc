//
// fastNLO author code for fnt1003 for midpoint ago 
...
void UserHHC::initfunc(unsigned int)
{
   ...
   for( int j = 0; j < nrap; j++) {
      ...
      for( int k = 0; k < npt[j]; k++) {
         ...
         if (j==1) xlimit[j][k] = xmin * 0.8;
         if (j==2) xlimit[j][k] = xmin * 0.85;
         if (j==3) xlimit[j][k] = xmin * 0.9;
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
   ...
   if (nj>=2) {
     ...
     double jpt[2];
     double jrap[2];
     if (pt1>ptlow && fabs(pj[ij1].prapidity())>0.1 && fabs(pj[ij1].prapidity())<0.7) {
       njet = 1;
       jpt[0] = pt1;
       jrap[0] = fabs(pj[ij2].prapidity());
     }
     if (pt2>ptlow && fabs(pj[ij2].prapidity())>0.1 && fabs(pj[ij2].prapidity())<0.7) {
       njet = njet+1;
       jpt[1] = jpt[0];
       jrap[1] = jrap[0];
       jpt[0] = pt2;
       jrap[0] = fabs(pj[ij1].prapidity());
     }

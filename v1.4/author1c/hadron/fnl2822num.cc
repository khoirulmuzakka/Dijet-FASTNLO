...
void UserHHC::initfunc(unsigned int)
{
  ...
  // Here: Leading jet binning no. 3, 4; only jets in |y| < 3.0
  // # of bins
  nrap = 2;
  double rapb[3] = { 2.5, 3.5, 4.5 };
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
  ...
  if (nj > 0) {
    ...
    //    double ptmax = pt1;
    // Drop the leading two jets since 2->2 is missing!
    double ptjet[2] = {pt3,pt4};
    
    // Loop over surviving jets, max. four, drop the leading two!
    if ( njet > 2 ) {
      for (int i=3; i<=min(4,njet); i++) {
    
	// --- determine y(=mult) and pT(=pT) bin
	// KR: Normalize to bin width in pT and |y|, but not multiplicity :-)
	double binwidth = 6.0;
	// Shift rapbin since the jets 1 and 2 will not be filled!
	int rapbin = i-3;
	int ptbin  = -1;
	for (int j = 0; j < npt[rapbin]; j++) {
	  if (pthigh[rapbin][j] <= ptjet[rapbin] && ptjet[rapbin] < pthigh[rapbin][j+1]) {
	    ptbin=j;
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
  ...
  // absolute order
  int npow = nord+2; // -> here "+2" for three-jet xsect
  WRITE(npow);

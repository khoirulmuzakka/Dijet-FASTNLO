...
void UserDIS::initfunc(unsigned int)
{
   ...
   unitfactor = 1000.0;  // for pb << used in ZEUS publication
   //   unitfactor = 1.0;  // for nb     

   //Set up binning  
   nrap   = 19;                 // no of bins in deltaeta2 aka rapidity
...
// ** deleta2:
// ve/cre bdt2(1) i 4
// ve/cre bsdt2(5) r 0. 0.8 1.6 2.4 3.8
   raphigh[0]=   0.0;         
   raphigh[1]=   0.8;         
   raphigh[2]=   1.6;         
   raphigh[3]=   2.4;         
...
// ** deleta1:
// ve/cre bdt1(1) i 4
// ve/cre bsdt1(6) r  0. 0.7 1.4 2.2  4. 
   raphigh[5]=   0.0;         
   raphigh[6]=   0.7;         
   raphigh[7]=   1.4;         
   raphigh[8]=   2.2;         
...
// // // ** deleta2 deleta1<1:
// // // ve/cre bdt2l(1) i 4
// // // ve/cre bsdt2l(5) r 0. 0.8 1.6 2.4 4.
   raphigh[10]=   0.0;         
   raphigh[11]=   0.8;         
   raphigh[12]=   1.6;         
   raphigh[13]=   2.4;         
...
// // // ** deleta2 deleta1>1:
// // // ve/cre bdt2g(1) i 4
// // // ve/cre bsdt2g(5) r 0. 0.7 1.4 2.1 3.4
   raphigh[15]=   0.0;         
   raphigh[16]=   0.7;         
   raphigh[17]=   1.4;         
   raphigh[18]=   2.1;         
   ...
   for( int j = 0; j < nrap; j++) {
      ...
      for( int k = 0; k < npt[j]; k++) {
         ...
         double  Q2min = 20.0;   // GeV^2
         double  Q2max = 100.0;  // GeV^2 
	 double dpt = (sqrt(Q2max)-sqrt(Q2min));
	 double lptlo = sqrt(Q2min) + dpt*0.;   // - starting points
	 double lpthi = sqrt(Q2max) - dpt*0.5;
...
void UserDIS::userfunc(const event_dis& p, const amplitude_dis& amp)
{
  //----- cuts in the laboratory & breit frame -----
   ...
   const int rapbin  =  0; 
   int rapbin1 = -1; // In this scenario, up to 3 plots can be filled per event
   int rapbin2 = -1;
   int rapbin3 = -1;
   ...
   // how many jets are left besides the forward jet ?
   ...
   for( int j = 5; j < 9; j++) {
      if (deta1 >= raphigh[j] && deta1 < raphigh[(j+1)]){
         rapbin2=j;
   ...
   if(deta1<1.0){
      for( int j = 10; j < 14; j++) {
         if (deta2 >= raphigh[j] && deta2 < raphigh[(j+1)]){
            rapbin3=j;
            binwidth3 = raphigh[(j+1)] - raphigh[j];
            break;
         }
      }
   }else{
   ...
   if(rapbin1==-1 && rapbin2==-1 && rapbin3==-1) return; // Anything to fill?
   ...
   for(int scalevar=0; scalevar<scalevarmax;scalevar++){
      double mur2 = murscale[scalevar]*murscale[scalevar]* Q2;
      double muf2 = mufscale[scalevar]*mufscale[scalevar]* Q2;
      ...
      double hmu   = log(log(murscale[scalevar]*sqrt(Q2)/mu0scale));
      ...
      for(int scalebin=0; scalebin<nscalebin;scalebin++){
         ...
         for( int i1 = 0; i1 < 4; i1++) {
            ...
            if (im<nxtot && im>=0) {
               ...
               if(rapbin3>=0)
                  weights[rapbin3][ptbin][im][scalevar][scalebin] +=  cefm[i1]  * deltascale / binwidth3 * wt;
            }
         }// end loop x interpolation

      }// end loop scale interpolation
   ...
   if (iref==1) {  // -> in reference mode
      double mur2 = murscale[refscale]*murscale[refscale] * Q2;
      double muf2 = mufscale[refscale]*mufscale[refscale] * Q2;
      ...
      if(rapbin3>=0)
         weights[rapbin3+nrap/2][ptbin][0][0][0] +=   wt/ binwidth3;

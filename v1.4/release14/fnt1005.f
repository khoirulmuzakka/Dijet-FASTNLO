...
*                            
* fastNLO user code  
*         -> special version for scenario fnt1005 which is
*            the ratio of two cross sections (fnt1005a, fnt1005b) 
*
...
*******************************************************************
      SUBROUTINE FT1005CC(FILENAME1, FILENAME2, 
     +     XMUR, XMUF, IPRINTFLAG, XSECT)
*-----------------------------------------------------------------
* fastNLO user code - main routine 
*
* input:
*   FILENAME1    name of 1st input table (630GeV)
*   FILENAME2    name of 2nd input table (1800GeV)
...
*-----------------------------------------------------------------
      IMPLICIT NONE
      INTEGER j, IPRINTFLAG
      CHARACTER*(*) FILENAME1, FILENAME2
      DOUBLE PRECISION xmur, xmuf, array1(20,3), array2(20,3),xsect(20,3)


      call FT1005aCC(FILENAME1, xmur,xmuf, 0, array1)
      call FT1005bCC(FILENAME2, xmur,xmuf, 0, array2)

      do j=1,20
         xsect(j,1) = array1(j,1)/array2(j,1)
         xsect(j,2) = (array1(j,1)+array1(j,2))/
     +        (array2(j,1)+array2(j,2)) - xsect(j,1)
         xsect(j,3) = (array1(j,1)+array1(j,2)+array1(j,3))/
     +        (array2(j,1)+array2(j,2)+array2(j,3)) -xsect(j,1)-xsect(j,2)
      enddo

      if (iprintflag.eq.1) then
         write(*,*) '--  fastNLO - results for ',
     +        'sigma(630GeV)/sigma(1800GeV) vs. xT'
         write(*,*) '--------  muf/mu0= ',xmuf,'    mur/mu0= ',xmur
         write(*,*) '--              from 0.0 to 0.5 in eta'
         write(*,*) '--    results:  LO          NLOcorr       ',
     +        '2-loop       total'
         do j=1,20
            write(*,*) 'xT bin No.',j,'  ',xsect(j,1),xsect(j,2),xsect(j,3),
     +           (xsect(j,1)+xsect(j,2)+xsect(j,3))
         enddo
         write(*,*)
      endif
...
*******************************************************************
      SUBROUTINE FT1005ACC(FILENAME, XMUR, XMUF, IPRINTFLAG, XSECT)
*-----------------------------------------------------------------
* fastNLO user code - main routine 
*
* input:
...
c - now get the PDFs ...
      call FT1005AGP(mufscale(ixmuf))
c - ... and multiply with perturbative coefficients 
      call FT1005AMT(xmur,ixmuf)
...
c - print results - if requested
      if (IPRINTFLAG.eq.1) call FT1005APR(xmur,IXMUF)
...
*******************************************************************
      SUBROUTINE FT1005AMT(xmur,ixmuf)
*-----------------------------------------------------------------
* MW 08/26/2005
*
...
*******************************************************************
      SUBROUTINE FT1005AGP(muffactor)
*-----------------------------------------------------------------
* MW 08/26/2005
*
...
c - now fill main PDF array - compute different lin. comb for diff sub-proc
               ...
               do k=1,NXTOT
                  ...
                  do l=1,nx2limit
                     nx=nx+1
                     call ft1005apl(ireaction,k,l,XPDF,H)
                     do m=1,Nsubproc
                        pdf(nbin,nx,m,p) = H(m)
                     enddo
                  enddo
...
*******************************************************************
      SUBROUTINE FT1005APL(ireact,i,j,XPDF,H)
* ---------------------------------------------------------------
* MW 03/27/06
...
*******************************************************************
      ...
      SUBROUTINE FT1005APR(XMUR,IMUFFLAG)
*-----------------------------------------------------------------
...
*******************************************************************
      SUBROUTINE FT1005BCC(FILENAME, XMUR, XMUF, IPRINTFLAG, XSECT)
*-----------------------------------------------------------------
* fastNLO user code - main routine 
*
* input:
...
c - now get the PDFs ...
      call FT1005BGP(mufscale(ixmuf))
c - ... and multiply with perturbative coefficients 
      call FT1005BMT(xmur,ixmuf)
...
c - print results - if requested
      if (IPRINTFLAG.eq.1) call FT1005BPR(xmur,IXMUF)
...
*******************************************************************
      SUBROUTINE FT1005BMT(xmur,ixmuf)
*-----------------------------------------------------------------
* MW 08/26/2005
*
* multiply the PDFs and the coefficients for a single scale setting
...
*******************************************************************
      SUBROUTINE FT1005BGP(muffactor)
*-----------------------------------------------------------------
* MW 08/26/2005
*
* read the PDFs in all (rap,pT) bins - at all xmax, xmin bins
...
c - now fill main PDF array - compute different lin. comb for diff sub-proc
               ...
               do k=1,NXTOT
                  ...
                  do l=1,nx2limit
                     nx=nx+1
                     call ft1005bpl(ireaction,k,l,XPDF,H)
                     do m=1,Nsubproc
                        pdf(nbin,nx,m,p) = H(m)
                     enddo
                  enddo
...
*******************************************************************
      SUBROUTINE FT1005BPL(ireact,i,j,XPDF,H)
* ---------------------------------------------------------------
* MW 03/27/06
* compute PDF linear combinations - for different reactions
*
...
*******************************************************************
      ...
      SUBROUTINE FT1005BPR(XMUR,IMUFFLAG)
*-----------------------------------------------------------------
* m. Wobisch  - print fastNLO cross section results
*
* input:

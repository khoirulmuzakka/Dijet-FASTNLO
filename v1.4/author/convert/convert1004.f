      PROGRAM CONVERT
c ---------------------------------------------------------------------
c --- M. Wobisch 2006/02/22
c --- 
c --- convert table 1004 to 1004 x-sect only
c ---      (remove 2nd eta bin)
c ---------------------------------------------------------------------
      implicit none
     ...
     +     xsc(9),xsc1(9),xsc2(9)
      ...
      write(*,*) ' '
      write(*,*) ' ***********************************************'
      write(*,*) ' *     convert table 1004 to 1004 x-sect only'
      write(*,*) ' * '
      write(*,*) ' * Markus Wobisch Feb 22, 2006'
      write(*,*) ' * '
      write(*,*) ' ***********************************************'
      IF ( IARGC().LT.2) then
         write(*,*) ' * input:  input-table   output-table'
         stop
      endif
...
c -------------------------------------------
      ...
      Integer Nrapmin, NPTMIN(20), NBINOFFSET
      include 'convert2003.inc'

      WRITE(*,*) ' write: ', filename

c - define variables -> to select the bins that we want to keep
      do i=1,20
         NPTMIN(i) = 1    ! smallest pT bin to be included
      enddo
      Nrapmin = 1
      Nbinoffset = 0           ! No of bins that are skipped
c      Nbinoffset = 31           ! No of bins that are skipped
c      Nrapmin = 2
      nrapidity = 1
c      NPTMIN(2) = 7
c      NPT(2) = 18       ! No. of pT bins t be used 

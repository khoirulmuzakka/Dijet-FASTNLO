//
// fastNLO author code for fnl2652:
//     ATLAS LHC Dijet Chi scenario, E_cms = 7 TeV
...
class UserHHC : public user_hhc
{
 ...
 private:
   ...
   pdf_cteq6 pdf;  //   pdf
   fj_ak_06 jetclus;   // jet algorithm
...
void UserHHC::initfunc(unsigned int)
{
   ...
   // # of bins
   nrap = 4;
   double rapb[5] = { 340., 520., 800., 1200., 7000.};
   ...
   //  		      12., 14., 16., 18., 20.};
   // ATLAS formula for lower border: Chi_n = exp(0.3 * n), n=0,10
   // BUT: Upper limit is Chi=30 and NOT exp(3.3) = 27.113 !
   for (unsigned int i=0; i<nrap/(iref+1); i++) {
     for (int j=0; j<npt[i]+1; j++) { 
       pthigh[i][j] = exp(0.3*j);
     }
     pthigh[i][npt[i]] = 30.;
   }
   ...
   for( int j = 0; j < nrap; j++) { 
	 ...
	 // With dymax=2*ymax: ptred = 0.0605677
	 // Analogous to fnl2622d with ymax: ptred = 0.1912172
	 double ptred = 0.1912172;
   ...
   cout << "    Scenario fnl2652:" << endl;
   cout  <<  "      ATLAS LHC Dijet Chi scenario, E_cms = 7 TeV,"  <<  endl;
   cout  <<  "      for fastjet anti-kT algo with R=0.6 in E-scheme"  <<  endl; 
   cout << " " << endl;
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   table << "ATLAS-LHC-Scenario" << endl;
   table << "Dijet_Chi" << endl;
   table << "anti-kT_R=0.6" << endl;
   table << "PLB 694 (2011)" << endl;
  ...
  //iproc
   ...
   //JetResol1
   double JetResol1 = 0.6; // distance parameter R
   WRITE(JetResol1);

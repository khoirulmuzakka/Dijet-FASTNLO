//
// fastNLO author code for fnl2622f:
...
void UserHHC::initfunc(unsigned int)
{
   ...
   // # of bins
   nrap = 9;
   double rapb[10] = {  400.,  600.,  800., 1000., 1200.,
   ...
   int nptb[9] = { 12, 12, 12, 12, 12, 12, 12, 12, 12 };
   npt = new int[nrap];
   for (unsigned int i=0; i<nrap/(iref+1); i++) {
     npt[i] = nptb[i]; 
   }
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   table << "d2sigma-dijet_dChidMjj_[pb_GeV]" << endl;
   table << "CMS_Collaboration" << endl;
   table << "Dijet_Chi" << endl;
   table << "anti-kT_R=0.5" << endl;
   table << "CMS-PAPER-EXO-11-017" << endl;

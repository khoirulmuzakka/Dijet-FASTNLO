//
// fastNLO author code for fnl2522 (normalization):
...
void UserHHC::initfunc(unsigned int)
{
   ...
   // Array for bin boundaries
   double ptb[6] = { 80.0, 110.0, 140.0, 200.0, 300.0, 3500. };
   for (unsigned int i=0; i<nrap/(iref+1); i++) {
   ...
   cout<<"    Scenario fnl2522norm:"<<endl;
...
//    cout << "nj, njet = " << nj << ", " << njet << endl;
   //DEBUGEND
   if (njet >= 2) {
     // Get the two to three surviving jets 
     int ij1 = 0;
     int ij2 = 0;
     int ij3 = 0;
     for (int i=1; i<=nj; i++) {
	 ...
	 pt3 = pttmp;
	 y3  = ytmp;
	 // For 3 jets with pt3 > pt2, check also pt3 (new pt2) vs. pt1 and exchange potentially
	 if (pt2 > pt1) {
	   int itmp     = ij1;
	   double pttmp = pt1;
	   double ytmp  = y1;
...
//      cout << "njet, pt1, pt2, yjjmax: " << njet << ", " << pt1 << ", " << pt2 << ", " << yjjmax << endl;
     ...
     if (pt1 > pthigh[0][0] && yjjmax < raphigh[1]) {
       //DEBUG

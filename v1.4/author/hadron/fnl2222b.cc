//
// fastNLO author code for fnl2222b:
...
void UserHHC::initfunc(unsigned int)
{
  ...
  cout << "    Scenario fnl2222b:" << endl;
  cout << "      CMS LHC Forward Jets Scenario, E_cms = 7 TeV," << endl;
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
  ...
  // lowest (pseudo-)rapidity for jets to be considered
  double yjmin = raphigh[0];
  ...
  for (int i = 1; i <= nj; i++) {
    ...
    if (pt > ptlow && yjmin < rap && rap < yjmax) {

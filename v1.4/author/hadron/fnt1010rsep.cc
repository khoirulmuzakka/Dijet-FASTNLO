//
// fastNLO author code for fnt1010
//         Run I   CDF dijet Chi distribution  hep-ex/9609011
//   rsep
// 
// last modification
...
void UserHHC::initfunc(unsigned int)
{
   ...
   //Set up binning  
   nrap = 5;  //             // No. of bins in rapidity -> here: Mass
   ...
   ptlow = 15.;          // absolute lowest pT-cut
   ...
   for( int j = 0; j < nrap; j++) { 
	 ...
	 cout<<"pT  "<<pthi[j][k]<<"  "<<ptlo[j][k]<<" "<<dpt<<"  "<<
	   (ptlo[j][k]+dpt*0.19)<<" "<<(pthi[j][k]-dpt*0.4)
         ...
         if (j==4) { // special definition in last - large - mass bin
	   murval[j][k][0] = ptlo[j][k] +dpt*0.08;  
	   murval[j][k][1] = pthi[j][k] -dpt*0.65;   // as for fnt1009 (D0) 
	 }
   ...
   cout<<"         *** scenario fnt1010 Rsep ***"<<endl;
   cout<<"             CDF Run I dijet Chi"<<endl;
   cout<<" "<<endl;
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
   ...
   if (nj>=2) {
     ...
     double dijetamax=max(fabs(pj[ij1].prapidity() ),fabs(pj[ij2].prapidity() ));

     // --- cuts
     if (pt1>ptlow && dijmass>241.0 && dijchi>1.0 && dijchi<5.0 && dijetamax<2.0 ) {
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
  ...
  //iproc
   ...
   //JetResol1
   double JetResol1 = 0.7;  // Rsep cone: R_cone
   WRITE(JetResol1);

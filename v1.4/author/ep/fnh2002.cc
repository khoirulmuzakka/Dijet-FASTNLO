...
//      fnh2002
// ZEUS incl jets  hep-ex/0608048   - using Bernstein Polynomials
// 
...
class UserDIS : public user_dis
{
 ...
 private:
   ...
   bounded_vector<lorentzvector<double> > pjb; 
...
void UserDIS::initfunc(unsigned int)
{
   ...
   //Set up binning  
   nrap   = 6;                 // no of bins in Q2 aka rapidity
  ...
  //Define binning in pt
   ...
   for( int j = 0; j < nrap; j++) {
      ...
      for( int k = 0; k < npt[j]; k++) {
         ...
         xlimit[j][k] *= (0.98+(5.0-double(j))*0.25);  // MW optimize xlim
         if (j<=1 || j==5) xlimit[j][k] *= 1.13;  // MW optimize xlim
         //hxlim[j][k] = -sqrt(-log10(xlimit[j][k]));  // fastNLO deafult
         hxlim[j][k] = log10(xlimit[j][k]);           // log(x) x-scheme
         xsmallest[j][k]=0.999;

         // - Setup the murval and mufval arrays - for scale bins
         double dpt = (pthigh[j][k+1]-pthigh[j][k]);
         double lptlo = pthigh[j][k] + dpt*0.05;   // - starting points
         double lpthi = pthigh[j][k+1] - dpt*0.2;
         if (j==5) lpthi = pthigh[j][k+1] - dpt*0.5;
         double llptlo = log(log(lptlo/mu0scale));     // - ln(ln((pt/mu0))
         double llpthi = log(log(lpthi/mu0scale)); 

         for(int m = 0; m < nscalebin; m++){   // -> only really need extreme values
	 ...
	 /*
         // - Determine bin width - and define two points between which we interpolate
	 double dpt = pthigh[j][k+1]-pthigh[j][k];
         murval[j][k][0] = pthigh[j][k] +dpt*0.05; 
         mufval[j][k][0] = murval[j][k][0];
         // Right point, scale down by 8% to get better interpolation of steeply falling function
         if(k==4){
	   murval[j][k][1] = pthigh[j][k+1] -dpt*0.6;
	   // last pt bin is HUGE, special treatment
         }else{
	   murval[j][k][1] = pthigh[j][k+1] -dpt*0.2;
         }
...
void UserDIS::userfunc(const event_dis& p, const amplitude_dis& amp)
{
  //----- cuts in the laboratory & breit frame -----
   ...
   const double etamax = 1.5;        //  pseudo-rapidity cut, Breit fram
   ...
   double s = 4.0*El*Eh;
   y = Q2/(s*xbj);
   double term1 = (1.0-y)*xbj*Eh;
   double term2 = y*El;
   double cosgamma = (term1-term2)/(term1+term2);
   //if (!(cosgamma>-0.7 && cosgamma<0.5)) return;
   if (fabs(cosgamma) > 0.65) return;
   ...
   for( int i = 1; i <= nj; i++) {
      ...
      if (pt>ptlow){ // check for low pt first, will fail most of the time
         double eta = pjb[i].prapidity();
         if(eta > etamin && eta < etamax){
            if(firstjet){
               // Calculate all quantities which rely on kinematics only one time per jet
               alem = xalpha_em(Q2);
               x = (p[-1]*p[0])/(p[-1]*p[hadron(0)]);
               //hx   = -sqrt(-log10(x)); // x binning  default x-scheme
               hx   = log10(x); // x binning    log(x) x-scheme
               hxone   = 0.0;
               ...
               for( int j = 0; j < nloop; j++) {
                  if (Q2 >= raphigh[j] && Q2 < raphigh[(j+1)]){
                     rapbin=j;
                     //                     rapbinwidth = raphigh[(j+1)] - raphigh[j];
                     rapbinwidth = 1.0; // ZEUS doesn't divide by Q2 bin size in this analysis
                     break;
                  }
               }
               firstjet = false;
            ...
            if (rapbin >=0 ) {              
               ...
               for( int j = 0; j < npt[rapbin]; j++) {
                  if (pt >= pthigh[rapbin][j] && pt < pthigh[rapbin][(j+1)]) {
                     ptbin=j;
                     binwidth=rapbinwidth*(pthigh[rapbin][(j+1)]-pthigh[rapbin][j]);
                     break;
                  }
               }
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   table << "dsigma-jet_dET(pb_GeV)" << endl;
   table << "hep-ex-0608048" << endl;
   table << "ZEUS_Collaboration" << endl;
   table << "-" << endl;
   table << "-" << endl;
  ...
  //iproc
   ...
   //ixscheme
   int ixscheme = 1;  //   1 log(x)   2 sqrt(log(1/x)
   WRITE(ixscheme);

//
// fastNLO author code for fnt1002 for midpoint algo - with Rsep=1.3
...
void UserHHC::initfunc(unsigned int)
{
   ...
   unitfactor = 1000000.0;  // D0 paper has numbers in fb

   //Set up binning  
   nrap   = 5;                 // No. of bins in rapidity (5 for D0) 
   ...
   for( int j = 0; j < nrap; j++) {
	 ...
	 xlimit[j][k] = xmin*0.84;
	 if (j==0) xlimit[j][k] = xmin*0.88; 
	 if (j==4) xlimit[j][k] = xmin*0.82;  
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   table << "d2sigma-jet_dET_deta_(fb_GeV)" << endl;
   table << "hep-ex-0011036" << endl;

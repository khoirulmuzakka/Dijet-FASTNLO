//
// fastNLO author code for fnt1007
//         Run I  CDF dijet mass  hep-ex/9912022
//             rsep=1.3 version
// 
// last modification
...
class UserHHC : public user_hhc
{
 ...
 private:
   ...
   pdf_cteq6 pdf;  //   pdf
   rsep_e_07 jetclus;   // jet algorithm
...
void UserHHC::initfunc(unsigned int)
{
   ...
   nxtot = 10;  // 12 was used in 1st ref-table <0.05%
   ...
   for( int j = 0; j < nrap; j++) { 
	 ...
	 double xmin = pthigh[j][k]*pthigh[j][k] / s; 
	 ...
	 xlimit[j][k] = xlimit[j][k]*0.99;
	 if (k==0) xlimit[j][k] = xlimit[j][k]*1.35;
	 if (k==1) xlimit[j][k] = xlimit[j][k]*1.23;
	 if (k==2) xlimit[j][k] = xlimit[j][k]*1.12;
   ...
   cout<<"         *** scenario fnt1007 ***"<<endl;
   cout<<"         (CDF Run I dijets - w/ rsep)"<<endl;
   cout<<"            (hep-ex/9912022)"<<endl;
   cout<<" "<<endl;
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
   ...
   if (nj>=2) {
     ...
     double costhstar = abs( tanh((pj[ij1].prapidity()-
				   pj[ij2].prapidity())/2.0));

     if (dijeta<2.0 && dijmass > 180.0 && costhstar<0.66666666) {
	 ...
	 if ( ptbin>=0 ) {
	   ...
	   if (xmin<xlimit[rapbin][ptbin]){
	     printf("Warning: xmin (%f) < xlimit (%f) at pt=%f GeV y=%f \n ",
		    xmin,xlimit[rapbin][ptbin],0.0,dijmass);
	     exit(1);
	   }
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   table << "dsigma-dijet_dMass_(pb_GeV)" << endl;
   table << "hep-ex-9912022" << endl;

//
// fastNLO author code for fnt1003 for midpoint algo  - with Rsep=1.3
//                             nxtot=10 -> faster for CTEQ fit
//
...
// 2006/01/19 MW - bug fix (forgot variable "nloop" - problem in ref-mode)
//             --> used for production
//
...
class UserHHC : public user_hhc
{
 ...
 private:
  ...
  vector< vector<double> > bicef;
   ...
   pdf_cteq6 pdf;  //   pdf
   rsep_et_07 jetclus;   // jet algorithm
...
void UserHHC::initfunc(unsigned int)
{
   ...
   //Set up binning  
   nrap   = 4;                 // No. of bins in rapidity (4 for CDF) 
   ...
   //Define binning in pT   - total 51 bins
   ...
   ptlow = 38.04;   // 38.04 for CDF dijets
   ...
   for( int j = 0; j < nrap; j++) {
      ...
      for( int k = 0; k < npt[j]; k++) {
         ...
         xlimit[j][k] = xmin;
         xlimit[j][k] = xlimit[j][k]*0.77; //  safety factor for ET scheme
         if (j==1) xlimit[j][k] = xmin * 0.82;
         if (j==2) xlimit[j][k] = xmin * 0.87;
         if (j==3) xlimit[j][k] = xmin * 0.91;
   ...
   cout<<"         *** scenario fnt1003 ***"<<endl;
   cout<<"              with Rsep=1.3 "<<endl;
   cout<<" "<<endl;
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
   ...
   // --- logic to identify correct jets ---
   ...
   if (nj>=2) {
     ...
     double jpt[2];
     double jrap[2];
     if (pt1>ptlow && abs(pj[ij1].prapidity())>0.1 && abs(pj[ij1].prapidity())<0.7) {
       njet = 1;
       jpt[0] = pt1;
       jrap[0] = abs(pj[ij2].prapidity());
     }
     if (pt2>ptlow && abs(pj[ij2].prapidity())>0.1 && abs(pj[ij2].prapidity())<0.7) {
       njet = njet+1;
       jpt[1] = jpt[0];
       jrap[1] = jrap[0];
       jpt[0] = pt2;
     ...
     for( int i = 0; i < njet; i++) {
	   ...
	   rapbin=j;
	   binwidth = 2.0*(raphigh[(j+1)] - raphigh[j]) * 2.0*0.6;
	 }
       }
       if (rapbin >=0 ) {          
         int ptbin  = -1;
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   table << "d3sigma-dijet_dET_deta1_deta2_(nb_GeV)" << endl;
   table << "hep-ex-0012013" << endl;
   table << "CDF_Collaboration" << endl;
   table << "-" << endl;
   table << "-" << endl;

  //iproc
   int iproc = 2; // dijets   - incl. jets =1
   WRITE(iproc);

   //ialgo
   int ialgo = 3; // rsep cone algo
   WRITE(ialgo);
   ...
   //JetResol2
   double JetResol2 = 1.3; // rsep cone: Rsep

//
// fastNLO author code for fnt200c
//         Run II   dijet Chi distribution
//
// 
// last modification
...
void UserHHC::initfunc(unsigned int)
{
   ...
   refscale = 0;   // which of the scalevariations is used in ref-table?
   ...
   //Set up binning  
   nrap   = 5;  //  ->50           // No. of bins in rapidity -> here: Mass
   ...
   for(int i=0;i<nrap;i++){
     raphigh[i+1]=raphigh[i]+150.0;    // these are dijet mass bins
   }
   raphigh[5]=1450.0;
   ...
   ptlow = 40.;          // absolute lowest pT-cut
   ...
   for( int j = 0; j < nrap; j++) { 
	 ...
	 cout<<"pT  "<<pthi[j][k]<<"  "<<ptlo[j][k]<<" "<<dpt<<"  "<<
	   (ptlo[j][k]+dpt*0.18)<<" "<<(pthi[j][k]-dpt*0.3)
	     <<endl;
         ...
         if (j==4) {
	   murval[j][k][0] = ptlo[j][k] +dpt*0.15;
	   murval[j][k][1] = pthi[j][k] -dpt*0.48;   // <<- 0.5 was checked!!
	 }
...
void UserHHC::userfunc(const event_hhc& p, const amplitude_hhc& amp)
{
   ...
   if (nj>=2) {
     ...
     double dijchi = exp( pj[ij1].rapidity() - pj[ij2].rapidity() );
     double yboost = (pj[ij1].rapidity() + pj[ij2].rapidity())/2.0;

     if (pt1>ptlow && dijchi>1.0 && yboost<1.5 && dijmass > 250.0) {
	 ...
	 if ( ptbin>=0 ) {
	   ...
	   if (xmin<xsmallest[rapbin][ptbin]*0.98){  // 0.98 reduces output
		...
		(itype==amplitude_hhc::nlo && nevents> 100000)){  // 5hh
	       cout<<" "<<endl;

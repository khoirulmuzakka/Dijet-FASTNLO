...
kt_et_10_dis::operator()(const event_dis& ev, double r0)
{
  ...
  for(int n = nt; n > 1; n--) {
    ...
    pmin = _M_pair(1,2); smin = _M_p[1].pt2;
    for(int i = 1; i <= n; i++) {
      if((tmp = _M_p[i].pt2) < smin) {
	smin = tmp;
	kmin = i;
      }
    ...
    //----- do the clustering -----
    if(smin*r2 < pmin) {
      _M_pj.push_back(_Vec_to_Lv(_M_p[kmin]));
  ...
  _M_pj.push_back(_Vec_to_Lv(_M_p[1]));

  return _M_pj;
}

double kt_et_10_dis::_M_pair(int i, int j)
{
  static const double pi = 3.14159265358979323846;
  static const double twopi = 6.28318530717958647692;
  
  double et2 = (_M_p[i].pt2 < _M_p[j].pt2 ? _M_p[i].pt2 : _M_p[j].pt2);
  ...
  return et2*(deta*deta+dphi*dphi);
}

void kt_et_10_dis::_M_merge(int i, int j)
{
  _M_p[i].eta *= _M_p[i].pt;  
  _M_p[i].eta += _M_p[j].eta*_M_p[j].pt;  

  _M_p[i].phi *= _M_p[i].pt;  
  _M_p[i].phi += _M_p[j].phi*_M_p[j].pt;  
    
  _M_p[i].pt += _M_p[j].pt;
  _M_p[i].pt2 = _M_p[i].pt*_M_p[i].pt;

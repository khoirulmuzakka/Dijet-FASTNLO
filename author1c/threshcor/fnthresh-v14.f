...
c 06/01/30 MW adopt to read tableformat version v1.4
c 06/05/31 MW implement scalebins
c 06/06/09 MW improve x integration (x^2 sampling)
c
c ---------------------------------------------------------------------
      implicit none
      include 'fnthresh-v14.inc'
...
c   e.g.               fnt1001midp  (33 bins):   4h
c                      fnt1002midp  (90 bins):   7h
c                      fnt2003plus (116 bins):  >12h  
c                      fnt200a     (613 bins): >>12h  
      nypt  = 16
      nptpt = 32
      nxpt  = 50 ! 90 <- was for x^3 sampling
      ns4pt = 60
c      ns4pt = 1

c - for fine pT binning - also fine y bins - as for LHC
c      nypt = 16
c      nptpt = 27
...
c      ns4pt = 10
      ...
      write(*,*) ' * '
      write(*,*) ' * Jan 5, 2006 version for fastNLO tableformat v1c'
      write(*,*) ' ***********************************************'
...
c - output y,pT bins
     ...
     +           ptarr(i,j),' -',ptarr(i,j+1),'  ',xlimit(i,j),
     +           murval(i,j,1),murval(i,j,nscalebin)
         enddo
      enddo
...
c - recognize the fastNLO scenario 
      ...
      elseif (nrap.eq.1 .and. npt(1).eq.12) then ! fnr0001 -> pb
         FAC1 = FAC1 * 1000d0
         write(*,*) ' ... switching to units of: pb  (for fnr0001)'
      elseif (nrap.eq.5 .and. npt(1).eq.28) then ! fnl0002 -> pb
         FAC1 = FAC1 * 1000d0
         write(*,*) ' ... switching to units of: pb  (for fnl0002)'
      else
         write(*,*) ' ... use default units of: nb'
      endif
...
c     +                 ,irap,ipt,i,norm
                  ...
                  if (norm.lt.0d0) then
                     write(*,*) ' no accessible y/pT range in bin:',irap,ipt
...
C - use importance sampling (quadratic)
                     xa = xamin + (dble(k-0.5d0)/dble(nxpt))**2d0 *(1d0-xamin)
                     xabin =(1d0-xamin)*(2d0*kd-1d0)/dble(nxpt)/dble(nxpt)*norm
C - use importance sampling (cubic)
c                        xa = xamin+(dble(k-0.5d0)/dble(nxpt))**3d0*(1d0-xamin)
c                        xabin = (1d0-xamin)*(3d0*kd*kd-3d0*kd+1d0)
c     +                       /dble(nxpt)/dble(nxpt)/dble(nxpt) *norm
c --
                        s4max = xa*(s+t)+u
                        XB0=-XA*T/(XA*S+U)
                        SH0=XA*XB0*S
...
c    ren. scale variation for threshold corrections is not possible!!)
                        if (nscalebin.eq.1) then ! fixed scales
                           muf = mufval(irap,ipt,1) * mufscale(iscale)
                           mur = murval(irap,ipt,1) * murscale(iscale)
                        elseif (nscalebin.ge.2) then
                           muf = pt * mufscale(iscale)
                           mur = pt * murscale(iscale)
                        endif
                        mur2=mur*mur
                        muf2=muf*muf
...
c - fill all contribution into the fastNLO table (1-loop and 2-loop)
                        call FILLTABLE(0,xa,xb0,pt,xsa,nbin,irap,ipt,iscale)
                        call FILLTABLE(1,xa,xb0,pt,xsb,nbin,irap,ipt,iscale)
                        call FILLTABLE(2,xa,xb0,pt,xsc,nbin,irap,ipt,iscale)
...
c - fill these contributions into the fastNLO table
                        call FILLTABLE(1,xa,xb0,pt,xsb1,nbin,irap,ipt,iscale)
                        call FILLTABLE(1,xa,xb,pt,xsb2,nbin,irap,ipt,iscale)
                        call FILLTABLE(2,xa,xb0,pt,xsc1,nbin,irap,ipt,iscale)
                        call FILLTABLE(2,xa,xb,pt,xsc2,nbin,irap,ipt,iscale)
...
C ----------------------------------------------------------
      SUBROUTINE FILLTABLE(nloop,xa,xb,pt,coef,nbin,nrapbin,nptbin,nscale)
c -------------------------------------------
c MW 
...
c                  0 Born, 1: LL, 2: NLL 
c
c MW 05/30/06 add scalebins
c -------------------------------------------
      implicit none
      integer nloop,nbin,nrapbin,nptbin,nscale,nf
      double precision xa,xb,pt,coef(9)
      double precision xmin,xmax,cf(7)
      double precision reweight, hxmin, hxmax, hxone, hxlimit, hxi, hxj, 
     +     delta, deltamax, deltamin, hmu, deltascale(2)
      integer ifirst,i,j,k,l, isub,nmin,nmax
         ...
         do i=1,nrap
            do j=1,npt(i)
               ...
               do k=1,Nscalevar
                  do l=1,Nscalebin
                     murvaltrans(i,j,k,l) = 
     +                    log(log(murscale(k)*murval(i,j,l)/mu0scale))
                     mufvaltrans(i,j,k,l) = 
     +                    log(log(mufscale(k)*mufval(i,j,l)/mu0scale))
                  enddo
               enddo
            enddo
         enddo
...
c - determine contributions for scalebins
      if (nscalebin.eq.1) then
         deltascale(1) = 1d0
         deltascale(2) = 0d0
      elseif (nscalebin.eq.2) then
         hmu = log(log(murscale(nscale)*pt/mu0scale))
         deltascale(1) = 1d0-(hmu-murvaltrans(nrapbin,nptbin,nscale,1))/
     +        (murvaltrans(nrapbin,nptbin,nscale,2)-
     +        murvaltrans(nrapbin,nptbin,nscale,1))
         deltascale(2) = 1d0-(murvaltrans(nrapbin,nptbin,nscale,2)-hmu)/
     +        (murvaltrans(nrapbin,nptbin,nscale,2)-
     +        murvaltrans(nrapbin,nptbin,nscale,1))
      else
...
c - convert from C++ to Fortran range (0,n-1) -> (1,n)
      nmin = nmin + 1
      nmax = nmax + 1
     ...
     +        - 0.5d0*cmin(4)*cmin(4)*cmin(4)
      ...
      do i=1,4
         do j=1,4
            ...
            total = total+bicef(i,j)
         enddo
      enddo
c - check that nothing is lost
      if (abs(total-1d0).gt.0.0000001d0) write(*,*) 'total=',total
      if (abs(deltascale(1)+deltascale(2)-1d0).gt.0.0000001d0) 
     +     write(*,*) 'deltascale=',(deltascale(1)+deltascale(2))

c === here: for xb>xa - swap sbprocesses 2,3
c          --> but not needed for threshold corrections
      if (cf(2).ne.cf(3)) write(*,*) ' coef 2<>3 : ',cf(2),cf(3)
...
c -       swap subprocesses 2,3 (but not needed for thresh. corr.)
     ...
     +                 table(nloop,nbin,imax,imin,nscale,isub,k) = 
     +                 table(nloop,nbin,imax,imin,nscale,isub,k) 
     +                 + bicef(i,j) * cf(isub) * deltascale(k)
               enddo 
            enddo
         enddo
...
c      CHARACTER*(255) label
      ...
      do i=1,Nord
         READ(2,*) powlabel(i)
         READ(2,*) CODELABEL(i)
         write(*,*) ">>>   contribution ",i,powlabel(i)
         write(*,*) ">>>             from ",codelabel(i)
      enddo
      READ(2,*) Nsep
      If (nsep.ne.Iseparator) goto 999
      do i=1,Nord
      ...
      READ(2,*) NSCALEBIN
      IF (nscalebin.gt.2) then
         write(*,*) ' >>> NSCALEBIN = ',nscalebin,' not yet implemented'
         write(*,*) '   > only NSCALEBIN = 1 or 2 allowed'
         stop
      endif
...
c -------------------------------------------
      ...
      integer nloop, i,j,k,l,m,n, isub, nbin, mord,mpow,mscalevar
      include 'fnthresh-v14.inc'
               ...
               do l=1,k
                  do isub=1,7
                     do m=1,mscalevar ! Nscalevar or one
                        do n=1,NSCALEBIN ! No. of Bins in Scale
c                          WRITE(2,*) 1d0
                           WRITE(2,*) table(nloop,nbin,k,l,m,isub,n)
                        enddo
                     enddo
                  enddo
               enddo
...
C  Q QBAR ==> G G
C
c      SIG(6)=8./3.*(4./9.*(T2+U2)/T/U-1.+2.*T*U/S2)/S
      SIG(6)=8./3.*(4./9.*(T2+U2)/T/U-  (T2+U2)/S2)/S    ! from ESW
C
C  G G ==> Q QBAR
C
c      SIG(7)=3./8.*(4./9.*(T2+U2)/T/U-1.+2.*T*U/S2)/S
      SIG(7)=3./8.*(4./9.*(T2+U2)/T/U-  (T2+U2)/S2)/S    ! from ESW
C
C  Q G ==> Q G
C
c      SIG(8)=(2.-1./9.-4./9.*T2/S/U-2.*S*U/T2)/S
      SIG(8)=((U2+S2)/T2-4./9.*(S2+U2)/S/U)/S  ! from ESW
C
C  G G ==> G G
C

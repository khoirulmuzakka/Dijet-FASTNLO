...
void UserDIS::initfunc(unsigned int)
{
   ...
   for( int j = 0; j < nrap; j++) {
      ...
      for( int k = 0; k < npt[j]; k++) {
         ...
         xlimit[j][k]= (Q2/s/ymax+2.0*pow(pthigh[j][k],2)/s); 
...
void UserDIS::userfunc(const event_dis& p, const amplitude_dis& amp)
{
  //----- cuts in the laboratory & breit frame -----
   ...
   const double etamax = 2.5;        //  pseudo-rapidity cut, Lab fra,e
   const double meanptcut = 8.5; // Cut on 1/2*(Et1+Et2)
   ...
   int rapbin = -1;
   double Q2 =0.;
   double meanpt = 0.; // average pt of dijets
   ...
   if(nj>3){
      printf("This usermodule is not prepared for more than 3 partons.\n");
      exit(1);
   }
   if(nj<2) return;
   double jetpt[3];
   ...
   //identify remaining jets after eta cut 
   for( int i = 1; i <= nj; i++) {
   ...
   // find the jets with the highest pt, calculate average pt
   switch(jetcount){
   case 0:
   case 1:
      return;
   ...
   case 3:
      if(jetpt[0]>jetpt[1]){
         if(jetpt[1]>jetpt[2]){
            ...
            meanpt = 0.5*(jetpt[0]+jetpt[2]);
         }
      }else{
         if(jetpt[0]>jetpt[2]){
            meanpt = 0.5*(jetpt[0]+jetpt[1]);
         }else{
            meanpt = 0.5*(jetpt[1]+jetpt[2]);
         }
      }
      break;
   default:
      printf("Inconsistency when finding dijets.\n");
      exit(1);
      break;
   }

   if(meanpt>meanptcut){ // Cut on pt sum of dijets
      Q2 = 2*(p[-2]*p[-1]);
      alem = xalpha_em(Q2);
      ...
      if (rapbin >=0 ) {              
         ...
         for( int j = 0; j < npt[rapbin]; j++) {
            if (meanpt >= pthigh[rapbin][j] && meanpt < pthigh[rapbin][(j+1)]) {
               ptbin=j;
...
#define WRITE(n) table.write(reinterpret_cast<char *>(&n),sizeof(n))
   ...
   table << "d2sigma-jet_d(avgET)_dQ2_(pb_GeV3)" << endl;
